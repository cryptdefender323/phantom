...
func HTTPStartSession(address string, pathPrefix string, opts *HTTPOptions) (*PhantomHTTPClient, error) {
	var client *PhantomHTTPClient
	var err error
	if !opts.ForceHTTP {
...
// PhantomHTTPClient - Helper struct to keep everything together
type PhantomHTTPClient struct {
	Origin     string
	PathPrefix string
	driver     HTTPDriver
	ProxyURL   string
...
func (s *PhantomHTTPClient) SessionInit() error {
...
func (s *PhantomHTTPClient) NonceQueryArgument(uri *url.URL, value uint64) *url.URL {
...
func (s *PhantomHTTPClient) OTPQueryArgument(uri *url.URL, value string) *url.URL {
	values := uri.Query()
...
func (s *PhantomHTTPClient) newHTTPRequest(method string, uri *url.URL, body io.Reader) *http.Request {
...
func (s *PhantomHTTPClient) DoPoll(req *http.Request) (*http.Response, []byte, error) {
...
// session key yet.
func (s *PhantomHTTPClient) establishSessionID(sessionInit []byte) error {
...
func (s *PhantomHTTPClient) ReadEnvelope() (*pb.Envelope, error) {
	if s.Closed {
...
func (s *PhantomHTTPClient) WriteEnvelope(envelope *pb.Envelope) error {
	if s.Closed {
		return ErrClosed
	}
...
func (s *PhantomHTTPClient) CloseSession() error {
	if s.Closed {
		return nil
	}
	if s.SessionID == "" {
...
func (s *PhantomHTTPClient) pathJoinURL(segments []string) string {
...
func (s *PhantomHTTPClient) parseSegments() *url.URL {
...
func (s *PhantomHTTPClient) startSessionURL() *url.URL {
	curl := s.parseSegments()
	return curl
}

// Must return at least a file name, path segments are optional
func (s *PhantomHTTPClient) randomPath(segments []string, filenames []string, exts []string) []string {
	genSegments := []string{}
	if 0 < len(segments) {
...
func httpClient(address string, opts *HTTPOptions) *PhantomHTTPClient {
...
func httpsClient(address string, opts *HTTPOptions) *PhantomHTTPClient {
