...
func buildServiceDetail(serviceName string, config mgr.Config) *phantompb.ServiceDetails {
	detail := &phantompb.ServiceDetails{Name: serviceName}
...
func ListServices(hostName string) ([]*phantompb.ServiceDetails, error) {
	var servicesList []*phantompb.ServiceDetails
...
func GetServiceDetail(hostName string, serviceName string) (*phantompb.ServiceDetails, error) {
	manager, err := connectToServiceManager(hostName, ReadOnlyServiceManagerPermissions)
	if err != nil {
		return nil, err
	}
