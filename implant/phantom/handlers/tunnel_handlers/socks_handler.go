...
func SocksReqHandler(envelope *phantompb.Envelope, connection *transports.Connection) {
...
func (s *socks) Write(b []byte) (n int, err error) {
	...
	socksTunnels.recordActivity(s.stream.TunnelID)
	data, err := proto.Marshal(&phantompb.SocksData{
	...
	// {{end}}
	s.conn.Send <- &phantompb.Envelope{
