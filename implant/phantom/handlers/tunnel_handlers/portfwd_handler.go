...
func PortfwdReqHandler(envelope *phantompb.Envelope, connection *transports.Connection) {
	portfwdReq := &phantompb.PortfwdReq{}
	err := proto.Unmarshal(envelope.Data, portfwdReq)
	if err != nil {
		// {{if .Config.Debug}}
		log.Printf("[portfwd] Failed to unmarshal protobuf %s", err)
		// {{end}}
		portfwdResp, _ := proto.Marshal(&phantompb.Portfwd{
