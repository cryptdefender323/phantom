...
func snapshotExecuteChildren() []*phantompb.ExecuteChild {
	executeChildrenMu.RLock()
	children := make([]*phantompb.ExecuteChild, 0, len(trackedExecuteChildren))
	for _, entry := range trackedExecuteChildren {
		children = append(children, &phantompb.ExecuteChild{
			Pid:       entry.PID,
			Path:      entry.Path,
...
func executeChildrenHandler(data []byte, resp RPCResponse) {
	req := &phantompb.ExecuteChildrenReq{}
