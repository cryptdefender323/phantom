...
import (
	...
	"github.com/cryptdefender3232/phantom/client/command"
...
func consoleCmd(con *console.PhantomClient) *cobra.Command {
	consoleCmd := &cobra.Command{
		Use:   "console",
		Short: "Start the phantom client console",
	}
...
func consoleRunnerCmd(con *console.PhantomClient, run bool) (pre, post func(cmd *cobra.Command, args []string) error) {
	pre = func(cmd *cobra.Command, _ []string) error {
		...
		var rpc rpcpb.PhantomRPCClient
		var ln *grpc.ClientConn

		// Don't clobber output when simply running an implant command from system shell.
		if run {
			rpc, ln, err = connectWithSpinner(os.Stdout, target, func(statusFn transport.ConnectStatusFn) (rpcpb.PhantomRPCClient, *grpc.ClientConn, error) {
		...
		return console.StartClient(con, rpc, ln, &console.ConnectionDetails{ConfigKey: configKey, Config: config}, command.ServerCommands(con, nil), command.PhantomCommands(con), run, rcScript)
	}
