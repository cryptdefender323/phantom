...
func WGPortFwdRmCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	stopReq, err := con.Rpc.WGStopPortForward(context.Background(), &phantompb.WGPortForwardStopReq{
		ID:      int32(fwdID),
...
func PortfwdIDCompleter(con *console.PhantomClient) carapace.Action {
	callback := func(_ carapace.Context) carapace.Action {
		...
		fwdList, err := con.Rpc.WGListForwarders(context.Background(), &phantompb.WGTCPForwardersReq{
