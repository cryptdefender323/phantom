...
func NetstatCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	netstat, err := con.Rpc.Netstat(context.Background(), &phantompb.NetstatReq{
...
func PrintNetstat(netstat *phantompb.Netstat, implantPID int32, activeC2 string, numeric bool, con *console.PhantomClient) {
	lookup := func(skaddr *phantompb.SockTabEntry_SockAddr) string {
		addr := skaddr.Ip
