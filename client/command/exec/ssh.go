...
func SSHCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	var (
		privKey []byte
		err     error
	)
	...
	sshCmd, err := con.Rpc.RunSSHCommand(context.Background(), &phantompb.SSHCommandReq{
		Username: username,
		Hostname: hostname,
		Port:     uint32(port),
		PrivKey:  privKey,
...
func PrintSSHCmd(sshCmd *phantompb.SSHCommand, con *console.PhantomClient) {
...
func tryCredsFromLoot(con *console.PhantomClient) (string, string, []byte) {
	var (
		username string
		password string
		privKey  []byte
