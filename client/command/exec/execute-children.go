...
func ExecuteChildrenCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	execChildren, err := con.Rpc.ExecuteChildren(context.Background(), &phantompb.ExecuteChildrenReq{
...
func PrintExecuteChildren(execChildren *phantompb.ExecuteChildren, con *console.PhantomClient) {
	if execChildren == nil || len(execChildren.Children) == 0 {
		con.PrintInfof("No tracked background execute children\n")
		return
	}

	children := append([]*phantompb.ExecuteChild(nil), execChildren.Children...)
