...
import (
	"github.com/cryptdefender3232/phantom/client/command/completers"
...
func Commands(con *console.PhantomClient) []*cobra.Command {
	...
	executeShellcodeCmd := &cobra.Command{
		...
		Short: "Executes the given shellcode in the phantom process",
	...
	psExecCmd := &cobra.Command{
		...
		Short: "Start a phantom service on a remote target",
	...
	flags.Bind("", false, psExecCmd, func(f *pflag.FlagSet) {
		f.StringP("service-name", "s", "Phantom", "name that will be used to register the service")
		f.StringP("service-description", "d", "Phantom implant", "description of the service")
		...
		f.StringP("custom-exe", "c", "", "custom service executable to use instead of generating a new Phantom")
