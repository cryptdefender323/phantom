...
func ExecuteCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	if token || hidden || ppid != 0 {
		...
		exec, err = con.Rpc.ExecuteWindows(context.Background(), &phantompb.ExecuteWindowsReq{
		...
		exec, err = con.Rpc.Execute(context.Background(), &phantompb.ExecuteReq{
			Request:        con.ActiveTarget.Request(cmd),
			Path:           cmdPath,
			Args:           args,
			Output:         captureOutput,
...
func HandleExecuteResponse(exec *phantompb.Execute, cmdPath string, hostName string, cmd *cobra.Command, con *console.PhantomClient) {
...
func PrintExecute(exec *phantompb.Execute, cmd *cobra.Command, con *console.PhantomClient) {
	ignoreStderr, _ := cmd.Flags().GetBool("ignore-stderr")
	stdout, _ := cmd.Flags().GetString("stdout")
	stderr, _ := cmd.Flags().GetString("stderr")
...
func combineCommandOutput(exec *phantompb.Execute, combineStdOut bool, combineStdErr bool) []byte {
...
func LootExecute(commandOutput []byte, lootName string, phantomCmdName string, cmdName string, hostName string, con *console.PhantomClient) {
	...
	fileName := fmt.Sprintf("%s_%s_%s_%s.log", phantomCmdName, hostName, shortCommandName, timeNow)
	if lootName == "" {
		lootName = fmt.Sprintf("[%s] %s on %s (%s)", phantomCmdName, shortCommandName, hostName, timeNow)
	}
...
func PrintExecutionOutput(executionOutput string, saveOutput bool, commandName string, hostName string, con *console.PhantomClient) {
	con.PrintInfof("Output:\n%s", executionOutput)
...
func SaveExecutionOutput(executionOutput string, commandName string, hostName string, con *console.PhantomClient) {
