...
func TestLoadAIStateCmdLoadsConversationOverRPC(t *testing.T) {
	...
	cmd := loadAIStateCmd(&console.PhantomClient{Rpc: rpcClient}, aiTargetSummary{}, "")
	msg := cmd()
...
func TestLoadAIStateCmdCreatesConversationWhenNoneExist(t *testing.T) {
	...
	msg := loadAIStateCmd(&console.PhantomClient{Rpc: rpcClient}, aiTargetSummary{}, "")()
...
func TestSubmitPromptCmdCreatesConversationAndSavesUserMessage(t *testing.T) {
	...
	msg := submitPromptCmd(
		&console.PhantomClient{Rpc: rpcClient},
...
type aiRPCServer struct {
	rpcpb.UnimplementedPhantomRPCServer
...
func newAITestRPCClient(t *testing.T, srv rpcpb.PhantomRPCServer) (rpcpb.PhantomRPCClient, func()) {
	t.Helper()

	listener := bufconn.Listen(1024 * 1024)
	grpcServer := grpc.NewServer()
	rpcpb.RegisterPhantomRPCServer(grpcServer, srv)
	...
	return rpcpb.NewPhantomRPCClient(conn), cleanup
}
