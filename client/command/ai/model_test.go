...
func TestRenderHeaderUsesSingleCompactRow(t *testing.T) {
	...
	for _, fragment := range []string{"PHANTOM AI", "synced", "alice", "Session demo"} {
		if !strings.Contains(rendered, fragment) {
...
func TestComposerCtrlSOpensTargetSelectModal(t *testing.T) {
	...
	model := newAIModel(&console.PhantomClient{Rpc: rpcClient}, aiContext{
...
func TestThinkingLevelModalSavesConversationOverride(t *testing.T) {
	...
	model := newAIModel(&console.PhantomClient{Rpc: rpcClient}, aiContext{}, nil)
	model.loading = false
...
func TestAIProgramAnimatesWhileSubmitPending(t *testing.T) {
	...
	inner := newAIModel(&console.PhantomClient{Rpc: rpcClient}, aiContext{
		connection: aiConnectionSummary{Operator: "alice"},
	}, nil)
	inner.loading = false
