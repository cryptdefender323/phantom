...
import (
	...
	aithinking "github.com/cryptdefender3232/phantom/client/spin/thinking"
	clienttheme "github.com/cryptdefender3232/phantom/client/theme"
	"github.com/cryptdefender3232/phantom/protobuf/clientpb"
	"github.com/cryptdefender3232/phantom/protobuf/commonpb"
	"github.com/cryptdefender3232/phantom/util/clientpbutil"
...
func newAIModel(con *console.PhantomClient, ctx aiContext, listener <-chan *clientpb.Event) *aiModel {
	return &aiModel{
...
func (m *aiModel) renderTooSmall() string {
	lines := []string{
		m.styles.badge.Render("PHANTOM AI"),
		"",
...
func (m *aiModel) renderInputContent(width int) string {
	if len(m.input) == 0 {
		placeholder := truncateText("Ask Phantom AI about the active target...", maxInt(1, width))
		if m.focus == aiFocusComposer {
			if width == 1 {
				return m.styles.cursor.Render(" ")
			}
...
func loadAIStateCmd(con *console.PhantomClient, target aiTargetSummary, selectedID string) tea.Cmd {
	return loadAIStateWithStatusCmd(con, target, selectedID, "")
}

func loadAIStateWithStatusCmd(con *console.PhantomClient, target aiTargetSummary, selectedID string, baseStatus string) tea.Cmd {
	return func() tea.Msg {
...
func loadAITargetSelectionOptionsCmd(con *console.PhantomClient, activeTarget aiTargetSummary, conversationTargetSessionID string, conversationTargetBeaconID string) tea.Cmd {
	return func() tea.Msg {
		if con == nil || con.Rpc == nil {
...
func createConversationCmd(con *console.PhantomClient, target aiTargetSummary, provider string, model string, title string, systemPrompt string) tea.Cmd {
	return func() tea.Msg {
		if con == nil || con.Rpc == nil {
			return aiAsyncErrMsg{err: fmt.Errorf("AI RPC client is unavailable")}
		}
...
func deleteConversationCmd(con *console.PhantomClient, conversationID string, selectedID string, status string) tea.Cmd {
	return func() tea.Msg {
		if con == nil || con.Rpc == nil {
			return aiAsyncErrMsg{err: fmt.Errorf("AI RPC client is unavailable")}
		}
...
func updateConversationThinkingLevelCmd(con *console.PhantomClient, conversation *clientpb.AIConversation, thinkingLevel string) tea.Cmd {
	return func() tea.Msg {
		if con == nil || con.Rpc == nil {
			return aiAsyncErrMsg{err: fmt.Errorf("AI RPC client is unavailable")}
		}
...
func updateConversationTargetCmd(con *console.PhantomClient, conversation *clientpb.AIConversation, target aiTargetSummary) tea.Cmd {
	return func() tea.Msg {
		if con == nil || con.Rpc == nil {
			return aiAsyncErrMsg{err: fmt.Errorf("AI RPC client is unavailable")}
		}
...
func submitPromptCmd(con *console.PhantomClient, target aiTargetSummary, conversation *clientpb.AIConversation, provider string, model string, systemPrompt string, prompt string) tea.Cmd {
	return func() tea.Msg {
		return submitPromptMsg(con, target, conversation, provider, model, systemPrompt, prompt)
	}
}

func submitPromptMsg(con *console.PhantomClient, target aiTargetSummary, conversation *clientpb.AIConversation, provider string, model string, systemPrompt string, prompt string) tea.Msg {
	if con == nil || con.Rpc == nil {
		return aiAsyncErrMsg{err: fmt.Errorf("AI RPC client is unavailable")}
	}
