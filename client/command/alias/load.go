...
// AliasesLoadCmd - Locally load a alias into the Phantom shell.
func AliasesLoadCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	dirPath := args[0]
...
// LoadAlias - Load an alias into the Phantom shell from a given directory.
func LoadAlias(manifestPath string, cmd *cobra.Command, con *console.PhantomClient) (*AliasManifest, error) {
	...
	longHelp.WriteString("https://github.com/cryptdefender3232/phantom/wiki/Aliases-&-Extensions#aliases-command-parsing")
	...
	if aliasManifest.IsAssembly {
		...
		f.BoolP("in-process", "i", false, "Run in the current phantom process")
...
func runAliasCommand(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	if aliasManifest.IsAssembly {
		...
		executeAssemblyResp, err := con.Rpc.ExecuteAssembly(context.Background(), &phantompb.ExecuteAssemblyReq{
		...
		msg := fmt.Sprintf("Executing %s %s ...", cmd.Name(), extArgsStr)
		con.SpinUntil(msg, ctrl)
		spawnDllResp, err := con.Rpc.SpawnDll(context.Background(), &phantompb.InvokeSpawnDllReq{
			Request:     con.ActiveTarget.Request(cmd),
			Args:        args,
			Data:        binData,
		...
		msg := fmt.Sprintf("Executing %s %s ...", cmd.Name(), extArgsStr)
		con.SpinUntil(msg, ctrl)
		sideloadResp, err := con.Rpc.Sideload(context.Background(), &phantompb.SideloadReq{
			Request:     con.ActiveTarget.Request(cmd),
			Args:        args,
			Data:        binData,
...
func PrintSpawnDLLOutput(cmdName string, schema *packages.OutputSchema, spawnDllResp *phantompb.SpawnDll, outFilePath *os.File, con *console.PhantomClient) {
	var result string
...
func PrintSideloadOutput(cmdName string, schema *packages.OutputSchema, sideloadResp *phantompb.Sideload, outFilePath *os.File, con *console.PhantomClient) {
	var result string
...
func PrintAssemblyOutput(cmdName string, schema *packages.OutputSchema, execAsmResp *phantompb.ExecuteAssembly, outFilePath *os.File, con *console.PhantomClient) {
	var result string
