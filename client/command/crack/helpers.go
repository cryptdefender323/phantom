...
func CrackHcstat2Completer(con *console.PhantomClient) carapace.Action {
...
func CrackWordlistCompleter(con *console.PhantomClient) carapace.Action {
	return carapace.ActionCallback(func(c carapace.Context) carapace.Action {
		hcstat2, err := con.Rpc.CrackFilesList(context.Background(), &clientpb.CrackFile{Type: clientpb.CrackFileType_MARKOV_HCSTAT2})
		if err != nil {
			return carapace.ActionMessage("failed to fetch crack files: %s", err.Error())
...
func CrackRulesCompleter(con *console.PhantomClient) carapace.Action {
	return carapace.ActionCallback(func(c carapace.Context) carapace.Action {
		hcstat2, err := con.Rpc.CrackFilesList(context.Background(), &clientpb.CrackFile{Type: clientpb.CrackFileType_MARKOV_HCSTAT2})
		if err != nil {
			return carapace.ActionMessage("failed to fetch crack files: %s", err.Error())
