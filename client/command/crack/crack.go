...
func CrackCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func CrackStationsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	crackers, err := con.Rpc.Crackstations(context.Background(), &commonpb.Empty{})
	if err != nil {
		con.PrintErrorf("%s\n", err)
		return
...
func PrintNoCrackstations(con *console.PhantomClient) {
	con.PrintInfof("No crackstations connected to server\n")
}

func AreCrackersOnline(con *console.PhantomClient) bool {
	crackers, err := con.Rpc.Crackstations(context.Background(), &commonpb.Empty{})
	if err != nil {
		return false
	}
...
func PrintCrackers(crackers []*clientpb.Crackstation, con *console.PhantomClient, showBenchmarks bool) {
...
func printCracker(cracker *clientpb.Crackstation, index int, con *console.PhantomClient, showBenchmarks bool) {
	tw := table.NewWriter()
...
func printBenchmarks(cracker *clientpb.Crackstation, con *console.PhantomClient) {
	tw := table.NewWriter()
	tw.SetStyle(settings.GetTableStyle(con))
