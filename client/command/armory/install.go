...
func ArmoryInstallCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func installBundle(bundle *ArmoryBundle, armoryPK string, forceInstallation bool, clientConfig ArmoryHTTPConfig, con *console.PhantomClient) {
	installList := []string{}
...
func installPackageByName(name, armoryPK string, forceInstallation, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.PhantomClient) error {
	pendingPackages := make(map[string]string)
...
func installAliasPackage(entry *pkgCacheEntry, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.PhantomClient) error {
	if entry == nil {
...
func installExtensionPackage(entry *pkgCacheEntry, promptToOverwrite bool, clientConfig ArmoryHTTPConfig, con *console.PhantomClient) error {
	if entry == nil {
		return errors.New("package not found")
	}
	repoURL, err := url.Parse(entry.RepoURL)
...
func writeArmoryTempFile(data []byte) (string, error) {
	...
	tmpFile, err := os.CreateTemp("", "phantom-armory-")
	if err != nil {
		return "", err
	}
