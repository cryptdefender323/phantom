...
*/
func PerformDownload(remotePath string, fileName string, cmd *cobra.Command, con *console.PhantomClient) (*phantompb.Download, error) {
	ctrl := make(chan bool)
	con.SpinUntil(fmt.Sprintf("%s -> %s", fileName, "loot"), ctrl)
	download, err := con.Rpc.Download(context.Background(), &phantompb.DownloadReq{
...
func SendLootMessage(loot *clientpb.Loot, con *console.PhantomClient) {
...
func LootDownload(download *phantompb.Download, lootName string, fileType clientpb.FileType, cmd *cobra.Command, con *console.PhantomClient) {
...
func LootText(text string, lootName string, lootFileName string, fileType clientpb.FileType, con *console.PhantomClient) {
	lootMessage := CreateLootMessage(con.ActiveTarget.GetHostUUID(), lootFileName, lootName, fileType, []byte(text))
	SendLootMessage(lootMessage, con)
}

func LootBinary(data []byte, lootName string, lootFileName string, fileType clientpb.FileType, con *console.PhantomClient) {
...
func LootAddRemoteCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
