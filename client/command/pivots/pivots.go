...
func PivotsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session := con.ActiveTarget.GetSessionInteractive()
	if session == nil {
		return
	}
	pivotListeners, err := con.Rpc.PivotSessionListeners(context.Background(), &phantompb.PivotListenersReq{
...
func PrintPivotListeners(pivotListeners []*phantompb.PivotListener, con *console.PhantomClient) {
	tw := table.NewWriter()
...
func PivotTypeToString(pivotType phantompb.PivotType) string {
	switch pivotType {
	case phantompb.PivotType_TCP:
		return "TCP"
	case phantompb.PivotType_UDP:
		return "UDP"
	case phantompb.PivotType_NamedPipe:
		return "Named Pipe"
	}
	return "Unknown"
}
