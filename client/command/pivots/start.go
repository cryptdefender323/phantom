...
func StartTCPListenerCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	listener, err := con.Rpc.PivotStartListener(context.Background(), &phantompb.PivotStartListenerReq{
		Type:        phantompb.PivotType_TCP,
...
func StartNamedPipeListenerCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session := con.ActiveTarget.GetSessionInteractive()
	if session == nil {
		return
	}
	...
	listener, err := con.Rpc.PivotStartListener(context.Background(), &phantompb.PivotStartListenerReq{
		Type:        phantompb.PivotType_NamedPipe,
		BindAddress: bind,
		Request:     con.ActiveTarget.Request(cmd),
		Options:     options,
	})
