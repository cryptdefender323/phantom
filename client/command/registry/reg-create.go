...
func RegCreateKeyCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	createKey, err := con.Rpc.RegistryCreateKey(context.Background(), &phantompb.RegistryCreateKeyReq{
		Hive:     hive,
		Path:     finalPath,
		Key:      key,
		Hostname: hostname,
...
func PrintCreateKey(createKey *phantompb.RegistryCreateKey, regPath string, key string, con *console.PhantomClient) {
