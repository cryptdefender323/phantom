...
func RegListSubKeysCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	regList, err := con.Rpc.RegistryListSubKeys(context.Background(), &phantompb.RegistrySubKeyListReq{
		Hive:     hive,
		Hostname: hostname,
		Path:     regPath,
...
func PrintListSubKeys(regList *phantompb.RegistrySubKeyList, hive string, regPath string, con *console.PhantomClient) {
...
func RegListValuesCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
	...
	regList, err := con.Rpc.RegistryListValues(context.Background(), &phantompb.RegistryListValuesReq{
		Hive:     hive,
		Hostname: hostname,
		Path:     regPath,
		Request:  con.ActiveTarget.Request(cmd),
...
func PrintListValues(regList *phantompb.RegistryValuesList, hive string, regPath string, con *console.PhantomClient) {
	if regList.Response != nil && regList.Response.Err != "" {
		con.PrintErrorf("%s\n", regList.Response.Err)
		return
	}
