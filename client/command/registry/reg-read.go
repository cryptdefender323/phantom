...
func getType(t string) (uint32, error) {
	...
	switch t {
	case "binary":
		res = phantompb.RegistryTypeBinary
	case "dword":
		res = phantompb.RegistryTypeDWORD
	case "qword":
		res = phantompb.RegistryTypeQWORD
	case "string":
		res = phantompb.RegistryTypeString
	default:
...
func RegReadCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	var (
		finalPath string
		key       string
	)
	...
	regRead, err := con.Rpc.RegistryRead(context.Background(), &phantompb.RegistryReadReq{
		Hive:     hive,
		Path:     finalPath,
		Key:      key,
		Hostname: hostname,
...
func PrintRegRead(regRead *phantompb.RegistryRead, con *console.PhantomClient) {
...
func writeHiveDump(data []byte, encoder string, fileName string, saveLoot bool, lootName string, lootType string, lootFileName string, con *console.PhantomClient) {
	var rawData []byte
	var err error
...
func RegReadHiveCommand(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
	...
	hiveDump, err := con.Rpc.RegistryReadHive(context.Background(), &phantompb.RegistryReadHiveReq{
		RootHive:      rootHive,
