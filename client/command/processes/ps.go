...
func PsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func PrintPS(os string, ps *phantompb.Ps, interactive bool, fullInfo bool, flags *pflag.FlagSet, con *console.PhantomClient) {
...
func findKnownSecurityProducts(ps *phantompb.Ps) []string {
...
func procRow(proc *commonpb.Process, cmdLine bool, fullInfo bool, con *console.PhantomClient) table.Row {
	session, beacon := con.ActiveTarget.GetInteractive()
...
func GetPIDByName(cmd *cobra.Command, name string, con *console.PhantomClient) int {
	ps, err := con.Rpc.Ps(context.Background(), &phantompb.PsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
