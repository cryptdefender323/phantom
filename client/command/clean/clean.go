...
// CleanCmd - Remove all profiles, beacons, sessions and implant builds (Builds and logs will still exist on disk in .phantom)
func CleanCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func removeImplantBuilds(con *console.PhantomClient) error {
...
func removeProfiles(con *console.PhantomClient) error {
...
func removeSessionsAndBeacons(con *console.PhantomClient) error {
	...
	for _, session := range sessions.Sessions {
		_, err := con.Rpc.Kill(context.Background(), &phantompb.KillReq{
...
func Command(con *console.PhantomClient) []*cobra.Command {
	return []*cobra.Command{{
		Use:   "clean",
		Short: "Remove all profiles, beacons, sessions, implant builds and HTTP profiles (Builds and logs will still exist on disk in .phantom)",
