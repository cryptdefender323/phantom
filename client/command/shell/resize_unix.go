...
func startPtyResizeWatcher(con *console.PhantomClient, cmd *cobra.Command, tunnelID uint64) func() {
	...
	go func() {
		...
		send := func() bool {
			...
			_, err = con.Rpc.ShellResize(context.Background(), &phantompb.ShellResizeReq{
				Request:  req,
				TunnelID: tunnelID,
				Rows:     uint32(rows),
				Cols:     uint32(cols),
