...
func ShellCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	if con.ActiveTarget.GetSession().OS != linux && con.ActiveTarget.GetSession().OS != darwin {
		noPty = true // Phantom's PTYs are only supported on linux/darwin
	}
...
func ShellLsCmd(_ *cobra.Command, con *console.PhantomClient, _ []string) {
	managed := shells.List()
	if len(managed) == 0 {
...
func ShellAttachCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	if len(args) < 1 {
...
func ShellKillCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	if len(args) < 1 {
	...
	if force && sh.Pid > 1 {
		terminate, err := con.Rpc.Terminate(context.Background(), &phantompb.TerminateReq{
...
func runInteractive(cmd *cobra.Command, shellPath string, noPty bool, con *console.PhantomClient, existing *managedShell) shellResult {
	con.Println()
	con.PrintInfof("Shell management: `shell ls`, `shell attach <id>`\n")
	con.PrintInfof("Escape: press Ctrl-] to return to the Phantom client\n")
	...
	if existing == nil {
		...
		rpcTunnel, err := con.Rpc.CreateTunnel(ctxTunnel, &phantompb.Tunnel{
		...
		shell, err := con.Rpc.Shell(context.Background(), &phantompb.ShellReq{
			Request:   con.ActiveTarget.Request(cmd),
			Path:      shellPath,
			EnablePTY: !noPty,
			Rows:      rows,
...
func backgroundCloseShell(con *console.PhantomClient, tunnelID uint64, sessionID string) {
...
func closeShellTunnel(con *console.PhantomClient, tunnelID uint64, sessionID string) error {
	core.GetTunnels().Close(tunnelID)
	if con == nil || con.Rpc == nil {
		return nil
	}

	_, err := con.Rpc.CloseTunnel(context.Background(), &phantompb.Tunnel{
		TunnelID:  tunnelID,
		SessionID: sessionID,
	})
	return err
