...
func ExtensionLoadCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	dirPath := args[0]
	...
	phantomMenu := con.App.Menu("implant")
	for _, extCmd := range manifest.ExtCommand {
		if CmdExists(extCmd.CommandName, phantomMenu.Command) {
...
func ExtensionRegisterCommand(extCmd *ExtCommand, cmd *cobra.Command, con *console.PhantomClient) {
...
func loadExtension(goos string, goarch string, checkCache bool, ext *ExtCommand, cmd *cobra.Command, con *console.PhantomClient) error {
	...
	if checkCache {
		extList, err := con.Rpc.ListExtensions(context.Background(), &phantompb.ListExtensionsReq{
...
func registerExtension(goos string, ext *ExtCommand, binData []byte, cmd *cobra.Command, con *console.PhantomClient) error {
	...
	if beac != nil && sess == nil {
		go func() {
			registerResp, err := con.Rpc.RegisterExtension(context.Background(), &phantompb.RegisterExtensionReq{
				Name:    name,
				Data:    binData,
				OS:      goos,
				Init:    ext.Init,
...
func loadDep(goos string, goarch string, depName string, cmd *cobra.Command, con *console.PhantomClient) error {
...
func runExtensionCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	var (
		err           error
		extensionArgs []byte
		extName       string
	...
	callExtResp, err := con.Rpc.CallExtension(context.Background(), &phantompb.CallExtensionReq{
		Name:    name,
		Export:  entryPoint,
		Args:    extensionArgs,
		Request: con.ActiveTarget.Request(cmd),
...
func PrintExtOutput(extName string, commandName string, outputSchema *packages.OutputSchema, callExtension *phantompb.CallExtension, con *console.PhantomClient) {
