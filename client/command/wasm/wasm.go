...
func WasmCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	execWasmReq := &phantompb.ExecWasmExtensionReq{
...
func isRegistered(name string, cmd *cobra.Command, con *console.PhantomClient) bool {
	...
	defer cancel()
	loaded, err := con.Rpc.ListWasmExtensions(grpcCtx, &phantompb.ListWasmExtensionsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
		return false
...
func idOf(con *console.PhantomClient) string {
...
func runNonInteractive(execWasmReq *phantompb.ExecWasmExtensionReq, con *console.PhantomClient) {
...
func runInteractive(cmd *cobra.Command, execWasmReq *phantompb.ExecWasmExtensionReq, con *console.PhantomClient) {
...
func registerWasmExtension(wasmFilePath string, cmd *cobra.Command, con *console.PhantomClient) error {
	grpcCtx, cancel := con.GrpcContext(cmd)
	defer cancel()
	...
	_, err = con.Rpc.RegisterWasmExtension(grpcCtx, &phantompb.RegisterWasmExtensionReq{
		Request: con.ActiveTarget.Request(cmd),
		Name:    filepath.Base(wasmFilePath),
		WasmGz:  data,
	})
...
func WasmLsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}
