...
func RevToSelfCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	session, beacon := con.ActiveTarget.GetInteractive()
	if session == nil && beacon == nil {
		return
	}

	revert, err := con.Rpc.RevToSelf(context.Background(), &phantompb.RevToSelfReq{
...
func PrintRev2Self(revert *phantompb.RevToSelf, con *console.PhantomClient) {
