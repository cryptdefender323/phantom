...
func SelectBeacon(con *console.PhantomClient) (*clientpb.Beacon, error) {
...
func GetBeacon(con *console.PhantomClient, beaconID string) (*clientpb.Beacon, error) {
	grpcCtx, cancel := con.GrpcContext(nil)
	defer cancel()
	beacons, err := con.Rpc.GetBeacons(grpcCtx, &commonpb.Empty{})
	if err != nil {
...
func GetBeacons(con *console.PhantomClient) (*clientpb.Beacons, error) {
	grpcCtx, cancel := con.GrpcContext(nil)
	defer cancel()
	beacons, err := con.Rpc.GetBeacons(grpcCtx, &commonpb.Empty{})
	if err != nil {
