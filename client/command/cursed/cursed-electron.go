...
func CursedElectronCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func avadaKedavraElectron(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient, cargs []string) *core.CursedProcess {
	...
	if electronProcess != nil {
		...
		con.PrintWarnf("Phantom will need to kill and restart the process in order to perform code injection.\n")
...
func checkElectronPath(electronExe string, _ *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient) (bool, error) {
...
func checkElectronProcess(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient) (*commonpb.Process, error) {
...
func startCursedElectronProcess(electronExe string, session *clientpb.Session, cmd *cobra.Command, con *console.PhantomClient, cargs []string) (*core.CursedProcess, error) {
	...
	electronExec, err := con.Rpc.Execute(context.Background(), &phantompb.ExecuteReq{
		Request: con.ActiveTarget.Request(cmd),
		Path:    electronExe,
		Args:    args,
		Output:  false,
