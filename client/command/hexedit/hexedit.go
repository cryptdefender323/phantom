...
func HexEditCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func downloadRemote(ctx context.Context, con *console.PhantomClient, cmd *cobra.Command, path string, maxBytes int64) (*phantompb.Download, error) {
	download, err := con.Rpc.Download(ctx, &phantompb.DownloadReq{
...
func uploadEditedContent(cmd *cobra.Command, con *console.PhantomClient, path string, data []byte) error {
	ctx, cancel := con.GrpcContext(cmd)
	defer cancel()
	...
	upload, err := con.Rpc.Upload(ctx, &phantompb.UploadReq{
		Request:     con.ActiveTarget.Request(cmd),
		Path:        path,
		Data:        uploadData,
		Encoder:     "gzip",
...
func waitForBeaconTaskResponse(ctx context.Context, con *console.PhantomClient, taskID string, out proto.Message) error {
...
func waitForBeaconTask(ctx context.Context, con *console.PhantomClient, taskID string) (*clientpb.BeaconTask, error) {
