...
func serverInfo(_ *cobra.Command, con *console.PhantomClient) {
...
func serverSwitch(_ *cobra.Command, con *console.PhantomClient) {
...
var consoleManagers sync.Map // *console.PhantomClient -> *consoleManager
...
func getConsoleManager(con *console.PhantomClient) *consoleManager {
	if con == nil {
...
func (m *consoleManager) syncActiveFromConsole(con *console.PhantomClient) {
	if m == nil || con == nil {
		return
	}
...
func newSwitchTUIModel(instances []*consoleInstance, activeID string, configKeys []string, counts map[string]int) switchTUIModel {
	...
	newSelect := huh.NewSelect[string]().
		...
		Description("Create a new console context from a profile in ~/.phantom-client/configs. Use / to filter.").
