...
func UpdateCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func VerboseVersionsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func updateAvailable(con *console.PhantomClient, client *http.Client, release *version.Release, saveTo string, force bool) {
	...
	if confirm {
		...
		publicKey, err := loadPhantomPublicKey()
		if err != nil {
			con.PrintErrorf("%s\n", err)
			return
		}
...
func downloadAssetWithSignature(con *console.PhantomClient, client *http.Client, asset *version.Asset, assets []version.Asset, saveTo string, publicKey minisign.PublicKey) error {
	if asset == nil {
...
func loadPhantomPublicKey() (minisign.PublicKey, error) {
	if strings.TrimSpace(PhantomPublicKey) == "" {
		return minisign.PublicKey{}, errors.New("minisign public key not set at build time")
	}
	var publicKey minisign.PublicKey
	if err := publicKey.UnmarshalText([]byte(PhantomPublicKey)); err != nil {
