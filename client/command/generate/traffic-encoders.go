...
func TrafficEncodersCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func DisplayTrafficEncoders(encoderMap *clientpb.TrafficEncoderMap, con *console.PhantomClient) {
	tw := table.NewWriter()
...
func TrafficEncodersAddCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	grpcCtx, cancel := con.GrpcContext(cmd)
	defer cancel()
...
func displayTrafficEncoderTestProgress(testID string, completed chan interface{}, con *console.PhantomClient) {
...
func clearLines(count int, con *console.PhantomClient) {
	for i := 0; i < count; i++ {
...
func displayTrafficEncoderTests(running bool, tests *clientpb.TrafficEncoderTests, con *console.PhantomClient) int {
	tw := table.NewWriter()
	tw.SetStyle(settings.GetTableStyle(con))
	tw.AppendHeader(table.Row{
		"Test",
...
func TrafficEncodersRemoveCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func SelectTrafficEncoder(con *console.PhantomClient) string {
