...
func ImplantsCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
...
func PrintImplantBuilds(builds *clientpb.ImplantBuilds, filters ImplantBuildFilter, con *console.PhantomClient) {
	tw := table.NewWriter()
	...
	for phantomName, config := range builds.Configs {
		...
		implantType := ""
		if builds.ResourceIDs[phantomName].Type != "" {
			implantType = builds.ResourceIDs[phantomName].Type
		}
		...
		tw.AppendRow(table.Row{
			...
			fmt.Sprintf("%v", builds.ResourceIDs[phantomName].Value),
			fmt.Sprintf("%v", builds.Staged[phantomName]),
		})
	}
...
func ImplantBuildNameCompleter(con *console.PhantomClient) carapace.Action {
...
func ImplantBuildByName(name string, con *console.PhantomClient) *clientpb.ImplantConfig {
	builds, err := con.Rpc.ImplantBuilds(context.Background(), &commonpb.Empty{})
	if err != nil {
		return nil
	}
	for phantomName, build := range builds.Configs {
		if phantomName == name {
			return build
		}
	}
