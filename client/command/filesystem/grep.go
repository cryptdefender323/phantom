...
func GrepCmd(cmd *cobra.Command, con *console.PhantomClient, args []string) {
	...
	grep, err := con.Rpc.Grep(context.Background(), &phantompb.GrepReq{
...
func printGrep(grep *phantompb.Grep, searchPattern string, searchPath string, cmd *cobra.Command, con *console.PhantomClient) {
...
func grepLineResult(positions []*phantompb.GrepLinePosition, line string, colorize bool, allowFormatting bool) string {
	var result string = ""
...
func printGrepResults(results map[string]*phantompb.GrepResultsForFile, colorize bool, allowFormatting bool) ([]string, int, []string) {
