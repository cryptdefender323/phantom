...
// HuhTheme returns a huh Theme derived from the current Phantom client theme.
//
...
func HuhTheme() huh.Theme {
	return huh.ThemeFunc(func(isDark bool) *huh.Styles {
		...
		current := Current()
		border := paletteMod(current.Default, 300)
		text := paletteMod(current.Default, 900)
		dim := paletteMod(current.Default, 500)
		faint := paletteMod(current.Default, 400)

		primary := paletteColor(current.Primary)
		accent := paletteColor(current.Secondary)

		success := paletteColor(current.Success)
		danger := paletteColor(current.Danger)
		...
		t.Focused.BlurredButton = t.Focused.BlurredButton.Foreground(text).Background(paletteMod(current.Default, 100))
...
func paletteColor(p Palette) color.Color {
	if p.Default != "" {
...
func paletteMod(p Palette, mod int) color.Color {
	if mod != 0 {
		if v, ok := p.Mods[mod]; ok && v != "" {
			return lipgloss.Color(v)
		}
	}
	return paletteColor(p)
}
