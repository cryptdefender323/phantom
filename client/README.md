...
Main command-line client for interacting with and administering Phantom servers. Provides the Cobra entrypoint and aggregates console, command, and transport subsystems.
...
- `cli/` – Defines the Cobra root command and CLI initialization for the Phantom client binary. Sets up persistent flags, environment bootstrapping, and command wiring.
- `command/` – Shared registration glue, scaffolding, and helpers for Phantom client console command groups. Hosts common utilities for binding Cobra commands to RPC-backed actions.
...
- `transport/` – Client-side transports and RPC wiring to communicate with Phantom servers. Builds authenticated gRPC clients, TLS dialers, and reconnect logic.
