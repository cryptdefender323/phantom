...
// ChannelProxy binds the Phantom Tunnel to a net.Conn object
...
func (p *ChannelProxy) dialImplant(ctx context.Context) (*TunnelIO, error) {
	...
	rpcTunnel, err := p.Rpc.CreateTunnel(ctx, &phantompb.Tunnel{
	...
	portfwdResp, err := p.Rpc.Portfwd(ctx, &phantompb.PortfwdReq{
		Request: &commonpb.Request{
			SessionID: p.Session.ID,
		},
		Host:     p.Host(),
		Port:     p.Port(),
		Protocol: phantompb.PortFwdProtoTCP,
		TunnelID: tunnel.ID,
	...
	if portfwdResp.Response != nil && portfwdResp.Response.Err != "" {
		p.Rpc.CloseTunnel(ctx, &phantompb.Tunnel{
			TunnelID:  tunnel.ID,
			SessionID: p.Session.ID,
		})
