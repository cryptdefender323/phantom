...
import (
	...
	"github.com/cryptdefender3232/phantom/util/leaky"
...
func (f *socksProxy) Start(tcpProxy *TcpProxy) error {
	...
	for {
		...
		rpcSocks, err := tcpProxy.Rpc.CreateSocks(context.Background(), &phantompb.Socks{
		...
		go connect(connection, proxy, &phantompb.SocksData{
...
func connect(conn net.Conn, stream rpcpb.PhantomRPC_SocksProxyClient, frame *phantompb.SocksData) {
