...
// PhantomMCPServer wraps the MCP server with Phantom RPC access for handlers.
type PhantomMCPServer struct {
	Rpc    rpcpb.PhantomRPCClient
	server *mcpserver.MCPServer
	logger *log.Logger
}

func newServer(cfg Config, rpc rpcpb.PhantomRPCClient, logger *log.Logger) *PhantomMCPServer {
...
func (s *PhantomMCPServer) listSessionsAndBeaconsHandler(ctx context.Context, _ mcpapi.CallToolRequest) (*mcpapi.CallToolResult, error) {
	if s.Rpc == nil {
