...
func Start(cfg Config, rpc rpcpb.PhantomRPCClient) error {
	return defaultManager.start(cfg, rpc)
}

// ServeStdio runs the MCP server over stdio using the provided configuration.
func ServeStdio(cfg Config, rpc rpcpb.PhantomRPCClient) error {
...
func (m *mcpManager) start(cfg Config, rpc rpcpb.PhantomRPCClient) error {
	cfg = cfg.WithDefaults()
	if err := cfg.Validate(); err != nil {
		return err
	}
