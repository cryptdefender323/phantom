...
const (
	...
	// - host: show "[host]" prefix on phantom-client, "[server]" on server console
	// - operator-host: show "[operator@host]" prefix on phantom-client, "[server]" on server console
	// - basic: show "phantom >" (no prefix)
...
const DefaultPromptTemplate = `{{- if .IsServer -}}{{ .Styles.Bold.Render "[server]" }} {{ .Styles.Underline.Render "phantom" }}{{ .Target.Suffix }} > {{- else -}}{{- if .Host -}}{{ .Styles.BoldPrimary.Render (printf "[%s]" .Host) }} {{- end -}}{{ .Styles.Underline.Render "phantom" }}{{ .Target.Suffix }} > {{- end -}}`
