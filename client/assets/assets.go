...
import (
	...
	ver "github.com/cryptdefender3232/phantom/client/version"
)

const (
	// PhantomClientDirName - Directory storing all of the client configs/logs
	PhantomClientDirName = ".phantom-client"

	versionFileName = "version"
	envVarName      = "PHANTOM_CLIENT_ROOT_DIR"
)

// GetRootAppDir - Get the Phantom app dir ~/.phantom-client/
func GetRootAppDir() string {
	value := os.Getenv(envVarName)
	var dir string
	if len(value) == 0 {
		user, _ := user.Current()
		dir = filepath.Join(user.HomeDir, PhantomClientDirName)
	} else {
		dir = value
	}
...
// GetClientLogsDir - Get the Phantom client logs dir ~/.phantom-client/logs/
...
// GetConsoleLogsDir - Get the Phantom client console logs dir ~/.phantom-client/logs/console/
...
// GetMCPLogsDir - Get the Phantom client MCP logs dir ~/.phantom-client/logs/mcp/
...
func Setup(force bool, echo bool) {
	...
	if force || localVer == "" || localVer != ver.GitCommit {
		if echo {
			fmt.Printf(`
Phantom  Copyright (C) 2026  Bishop Fox
