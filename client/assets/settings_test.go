...
func TestLoadSettingsMigratesLegacyJSON(t *testing.T) {
	...
	legacy := &ClientSettings{
		...
		PromptTemplate:    "{{.Operator}}@{{.Host}} phantom > ",
	}
	...
	if settings.PromptTemplate != "{{.Operator}}@{{.Host}} phantom > " {
		t.Fatalf("expected PromptTemplate %q, got %q", "{{.Operator}}@{{.Host}} phantom > ", settings.PromptTemplate)
	}
...
func TestLoadSettingsWritesDefault(t *testing.T) {
	...
	if settings.TableStyle != "PhantomDefault" {
		t.Fatalf("expected default TableStyle %q, got %q", "PhantomDefault", settings.TableStyle)
	}
