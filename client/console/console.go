...
import (
	...
	"github.com/cryptdefender3232/phantom/client/assets"
	consts "github.com/cryptdefender3232/phantom/client/constants"
	"github.com/cryptdefender3232/phantom/client/core"
	"github.com/cryptdefender3232/phantom/client/spin"
	"github.com/cryptdefender3232/phantom/client/theme"
	"github.com/cryptdefender3232/phantom/client/version"
	"github.com/cryptdefender3232/phantom/protobuf/clientpb"
	"github.com/cryptdefender3232/phantom/protobuf/commonpb"
	"github.com/cryptdefender3232/phantom/protobuf/rpcpb"
...
type PhantomClient struct {
	...
	backgroundRPC          rpcpb.PhantomRPCClient
	...
	jsonRemoteStream      rpcpb.PhantomRPC_ClientLogClient
	asciicastRemoteStream rpcpb.PhantomRPC_ClientLogClient
}

// NewConsole creates the phantom client (and console), creating menus and prompts.
...
func NewConsole(isServer bool) *PhantomClient {
	...
	// Implant menu.
	phantom := con.App.NewMenu(consts.ImplantMenu)
	phantom.Short = "Implant commands"
	phantom.Prompt().Primary = con.GetPrompt
	phantom.AddInterrupt(io.EOF, con.exitImplantMenu) // Ctrl-D
...
// Init requires a working RPC connection to the phantom server, and 2 different sets of commands.
...
func StartClient(con *PhantomClient, rpc rpcpb.PhantomRPCClient, grpcConn *grpc.ClientConn, details *ConnectionDetails, serverCmds, phantomCmds console.Commands, run bool, rcScript string) error {
	con.IsCLI = !run
	con.serverCmds = serverCmds
	con.phantomCmds = phantomCmds
	...
	phantom := con.App.Menu(consts.ImplantMenu)
	phantom.SetCommands(phantomCmds)
	...
	if rcScript != "" {
		...
		con.printf = fmt.Printf
		con.runRCScript(serverCmds, phantomCmds, rcScript)
...
func (con *PhantomClient) runRCScript(serverCmds, phantomCmds console.Commands, rcScript string) {
	...
	for scanner.Scan() {
		...
		if err := con.runRCLine(serverCmds, phantomCmds, line); err != nil {
...
func (con *PhantomClient) runRCLine(serverCmds, phantomCmds console.Commands, line string) error {
...
func (con *PhantomClient) applyConnectionHooksOnce() {
	con.connMu.Lock()
...
func (con *PhantomClient) startEventLoop(ctx context.Context, rpc rpcpb.PhantomRPCClient) {
	if rpc == nil {
		return
	}
...
func (con *PhantomClient) CreateEventListener() (string, <-chan *clientpb.Event) {
...
func (con *PhantomClient) SuppressEventNotifications() func() {
	if con == nil {
		return func() {}
	}
...
func (con *PhantomClient) eventNotificationsSuppressed() bool {
	return con != nil && con.suppressedEventNotifs.Load() > 0
}

func (con *PhantomClient) emitToastEvent(level string, message string) {
...
func (con *PhantomClient) emitConsoleNotification(level string, emphasized bool, format string, args ...any) {
...
func (con *PhantomClient) RemoveEventListener(listenerID string) {
...
func (con *PhantomClient) triggerEventListeners(event *clientpb.Event) {
...
func (con *PhantomClient) triggerReactions(event *clientpb.Event) {
...
func (con *PhantomClient) triggerBeaconTaskCallback(data []byte) {
...
func (con *PhantomClient) AddBeaconCallback(taskID string, callback BeaconTaskCallback) {
	con.BeaconTaskCallbacksMutex.Lock()
	defer con.BeaconTaskCallbacksMutex.Unlock()
	con.BeaconTaskCallbacks[taskID] = callback
}

func (con *PhantomClient) GetPrompt() string {
...
func (con *PhantomClient) PrintLogo() {
	...
	fmt.Println("All operators stay " + abilities[util.Intn(len(abilities))] + "\r")

	if con.Rpc == nil {
		fmt.Println(Warn + "Not connected to a server\r")
		fmt.Printf("%sClient %s\r\n", Info, version.FullVersion())
		fmt.Println(Info + "Welcome to Phantom, type 'help' for options\r")
		fmt.Println("")
		return
	}
...
func (con *PhantomClient) CheckLastUpdate() {
	now := time.Now()
...
func (con *PhantomClient) GetSession(arg string) *clientpb.Session {
...
func (con *PhantomClient) GetSessionsByName(name string) []*clientpb.Session {
	sessions, err := con.Rpc.GetSessions(context.Background(), &commonpb.Empty{})
	if err != nil {
		fmt.Printf("%s%s\n", Warn, err)
		return nil
...
func (con *PhantomClient) GetActiveSessionConfig() *clientpb.ImplantConfig {
...
func (con *PhantomClient) GetActiveBeaconConfig() *clientpb.ImplantConfig {
...
func (c *PhantomClient) exitConsole(_ *console.Console) {
...
func (c *PhantomClient) exitImplantMenu(_ *console.Console) {
	root := c.App.Menu(consts.ImplantMenu).Command
	root.SetArgs([]string{"background"})
	root.Execute()
}

func (con *PhantomClient) SpinUntil(message string, ctrl chan bool) {
...
func (con *PhantomClient) FormatDateDelta(t time.Time, includeDate bool, color bool) string {
...
func (con *PhantomClient) GrpcContext(cmd *cobra.Command) (context.Context, context.CancelFunc) {
	if cmd == nil {
...
func (con *PhantomClient) ExposeCommands() {
...
func (l asciiLogo) Render() string {
	...
	case logoStyleCyan:
		return lipgloss.NewStyle().Foreground(lipgloss.Color("6")).Bold(true).Render(l.art)
	default:
		return l.art
	}
}
