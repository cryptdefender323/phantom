...
type ConsoleClientLogger struct {
	name   string
	Stream rpcpb.PhantomRPC_ClientLogClient
}
...
func (con *PhantomClient) ensureJSONRemoteWriter() {
	con.connMu.Lock()
...
func (con *PhantomClient) ensureAsciicastRemoteWriter() {
	con.connMu.Lock()
	defer con.connMu.Unlock()
...
func (con *PhantomClient) refreshRemoteLogStreamsLocked() {
...
func (con *PhantomClient) setRemoteLogStreamsLocked(jsonStream, asciicastStream *ConsoleClientLogger) {
...
func (con *PhantomClient) ClientLogStream(name string) (*ConsoleClientLogger, error) {
	con.connMu.Lock()
	rpc := con.backgroundRPCClientLocked()
	con.connMu.Unlock()
	if rpc == nil {
...
func (con *PhantomClient) clientLogStream(rpc rpcpb.PhantomRPCClient, name string) (*ConsoleClientLogger, error) {
...
func (con *PhantomClient) setupLogger(writers ...io.Writer) {
...
func (con *PhantomClient) logCommand(args []string) ([]string, error) {
	if len(args) == 0 {
		return args, nil
	}
...
func (con *PhantomClient) setupAsciicastRecord(logFile *os.File, server io.Writer) {
	...
	if _, err := rand.Read(marker); err != nil {
		copy(marker, []byte(fmt.Sprintf("phantom-sync-%d", time.Now().UnixNano())))
	}
...
func (con *PhantomClient) copyStdoutPipe(src *os.File, dst io.Writer, done chan struct{}) {
	defer close(done)
	defer src.Close()
...
func (con *PhantomClient) ackStdoutSync(seq uint64) {
...
func (con *PhantomClient) clearStdoutSync(seq uint64) {
	con.stdoutSyncAcksMu.Lock()
	delete(con.stdoutSyncAcks, seq)
	con.stdoutSyncAcksMu.Unlock()
}

func (con *PhantomClient) syncOutputHook() error {
	con.syncOutput()
	return nil
}

func (con *PhantomClient) syncOutput() {
...
func (con *PhantomClient) FlushOutput() {
	if con.stdoutPipeWriter == nil {
		_ = os.Stdout.Sync()
		return
	}
...
func (con *PhantomClient) PrintAsyncResponse(resp *commonpb.Response) {
...
func (con *PhantomClient) Printf(format string, args ...any) {
...
func (con *PhantomClient) Println(args ...any) {
...
func (con *PhantomClient) PrintInfof(format string, args ...any) {
	logger := slog.New(con.jsonHandler)
...
func (con *PhantomClient) PrintSuccessf(format string, args ...any) {
	logger := slog.New(con.jsonHandler)
...
func (con *PhantomClient) PrintSuccess(args ...any) {
	if len(args) == 0 {
...
func (con *PhantomClient) PrintWarnf(format string, args ...any) {
	logger := slog.New(con.jsonHandler)
...
func (con *PhantomClient) PrintErrorf(format string, args ...any) {
	logger := slog.New(con.jsonHandler)
...
func (con *PhantomClient) PrintEventInfof(format string, args ...any) {
...
func (con *PhantomClient) PrintEventErrorf(format string, args ...any) {
	logger := slog.New(con.jsonHandler).With(slog.String("type", "event"))
...
func (con *PhantomClient) PrintEventSuccessf(format string, args ...any) {
	logger := slog.New(con.jsonHandler).With(slog.String("type", "event"))
