...
// In the phantom-client binary this is typically sourced from ~/.phantom-client/configs/*.
...
func (con *PhantomClient) CurrentConnection() (*ConnectionDetails, connectivity.State, bool) {
	con.connMu.Lock()
...
func (con *PhantomClient) SetConnection(rpc rpcpb.PhantomRPCClient, grpcConn *grpc.ClientConn, details *ConnectionDetails) error {
	con.connMu.Lock()
	defer con.connMu.Unlock()
	...
	if backgroundRPC != nil {
		wg.Add(1)
		go func(wg *sync.WaitGroup, rpc rpcpb.PhantomRPCClient) {
			defer wg.Done()
...
func (con *PhantomClient) CloseConnection() error {
	con.connMu.Lock()
	defer con.connMu.Unlock()
...
func (con *PhantomClient) detachConnectionLocked() {
...
func (con *PhantomClient) backgroundRPCClientLocked() rpcpb.PhantomRPCClient {
...
func (con *PhantomClient) refreshDedicatedCommandConnectionHook(args []string) ([]string, error) {
...
func (con *PhantomClient) refreshDedicatedCommandConnection() error {
	con.connMu.Lock()
	defer con.connMu.Unlock()
...
func (con *PhantomClient) monitorConnectionLost(ctx context.Context, conn *grpc.ClientConn) {
	if conn == nil {
		return
	}
