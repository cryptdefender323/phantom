...
function Write-FailureLogs {
    $logPath = Join-Path $env:PHANTOM_ROOT_DIR "logs\phantom.log"
...
function Get-ServerBinaryPath {
    $candidates = @(
        (Join-Path $PWD.Path "phantom-server.exe"),
        (Join-Path $PWD.Path "phantom-server")
    )
...
function Invoke-UnpackServerAssets {
    ...
    if ($serverBinary) {
        try {
            ...
            Write-Host "phantom-server unpack failed, falling back to go run ./server unpack --force"
        }
    }
...
try {
    ...
    $script:TestTmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("phantom-go-tests-" + [System.Guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $script:TestTmpRoot -Force | Out-Null
    $script:CreatedTestRoot = $true

    $env:PHANTOM_ROOT_DIR = Join-Path $script:TestTmpRoot "phantom"
    $env:PHANTOM_CLIENT_DIR = Join-Path $script:TestTmpRoot "phantom-client"
    $env:PHANTOM_CLIENT_ROOT_DIR = $env:PHANTOM_CLIENT_DIR
    ...
    @(
        $env:PHANTOM_ROOT_DIR,
        $env:PHANTOM_CLIENT_DIR,
        $env:HOME,
        $env:XDG_CONFIG_HOME,
        $env:XDG_CACHE_HOME,
        $env:XDG_DATA_HOME,
    ...
    $phantomGoBin = Join-Path $env:PHANTOM_ROOT_DIR "go\bin"
    if (-not $isWindowsPlatform) {
        $env:PATH = [string]::Join([System.IO.Path]::PathSeparator, @($phantomGoBin, $env:PATH))
    }

    Write-Host "Using isolated temp directories:"
    Write-Host "  PHANTOM_ROOT_DIR=$env:PHANTOM_ROOT_DIR"
    Write-Host "  PHANTOM_CLIENT_DIR=$env:PHANTOM_CLIENT_DIR"
    ...
    if ($UnitOnly) {
        ...
        Invoke-TestStep -Name "./server/c2 (e2e yamux)" -Action {
            Invoke-ExternalCommand -FilePath "go" -Arguments @("test", "-tags=server,$tags,phantom_e2e", "./server/c2", "-run", "Test(MTLS|WG)Yamux_", "-count=1")
        }
        Invoke-TestStep -Name "./server/c2 (e2e dns)" -Action {
            Invoke-ExternalCommand -FilePath "go" -Arguments @("test", "-tags=server,$tags,phantom_e2e", "./server/c2", "-run", "TestDNS_", "-count=1")
        }
    }

    if (-not $SkipGenerate) {
        $generateGoMaxProcs = if ($env:PHANTOM_GENERATE_GOMAXPROCS) { $env:PHANTOM_GENERATE_GOMAXPROCS } else { "2" }
        $generateGoP = if ($env:PHANTOM_GENERATE_GO_P) { $env:PHANTOM_GENERATE_GO_P } else { "1" }
        $generateTestParallel = if ($env:PHANTOM_GENERATE_TEST_PARALLEL) { $env:PHANTOM_GENERATE_TEST_PARALLEL } else { "1" }
