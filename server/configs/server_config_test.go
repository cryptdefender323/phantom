...
func TestServerConfigParsesYAML(t *testing.T) {
	t.Setenv("PHANTOM_ROOT_DIR", t.TempDir())
...
`)
	...
	if config.AI == nil || config.AI.OpenRouter == nil || config.AI.OpenRouter.APIKey != "openrouter-key" || config.AI.OpenRouter.UserAgent != "phantom-test/1.0" {
