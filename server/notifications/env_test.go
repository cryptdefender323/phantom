...
func TestResolveEnvString(t *testing.T) {
	t.Setenv("PHANTOM_NOTIFY_TEST", "secret")

	got, ok, found := resolveEnvString("$PHANTOM_NOTIFY_TEST")
	...
	got, ok, found = resolveEnvString("$PHANTOM_NOTIFY_MISSING")
	if !ok || found || got != "" {
...
func TestExpandEnvConfig(t *testing.T) {
	t.Setenv("PHANTOM_NOTIFY_SLACK_TOKEN", "slack-secret")
	t.Setenv("PHANTOM_NOTIFY_SLACK_CHANNEL", "C123")
	t.Setenv("PHANTOM_NOTIFY_WEBHOOK", "https://example.com/hook")
	t.Setenv("PHANTOM_NOTIFY_HEADER", "Bearer 123")
	...
	cfg := &configs.NotificationsConfig{
		...
		Services: &configs.NotificationsServicesConfig{
			Slack: &configs.SlackConfig{
				...
				APIToken:                  "$PHANTOM_NOTIFY_SLACK_TOKEN",
				Channels:                  []string{"$PHANTOM_NOTIFY_SLACK_CHANNEL"},
			},
			HTTP: &configs.HTTPConfig{
				NotificationServiceConfig: configs.NotificationServiceConfig{Enabled: true},
				Webhooks: []configs.HTTPWebhookConfig{
					{
						URL: "$PHANTOM_NOTIFY_WEBHOOK",
						Headers: map[string]string{
							"Authorization": "$PHANTOM_NOTIFY_HEADER",
						},
					},
				},
			},
