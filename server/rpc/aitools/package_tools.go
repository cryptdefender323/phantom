...
import (
	...
	"github.com/cryptdefender3232/phantom/client/packages"
...
func packageToolDefinitions() []serverai.AgenticToolDefinition {
	return []serverai.AgenticToolDefinition{
		{
			Name:        "search_aliases",
			Description: "Search server-side AI aliases copied into ~/.phantom/ai/aliases. Results include manifest details and, when a target is selected, compatibility for that session or beacon.",
		...
		},
		{
			...
			Description: "Search server-side AI extensions copied into ~/.phantom/ai/extensions. Results include manifest details, dependency availability, and target compatibility when a target is selected.",
			Parameters: map[string]any{
				"type": "object",
				"properties": mergeSchemaProperties(map[string]any{
		...
		{
			Name:        "execute_alias",
			Description: "Execute a server-side AI alias from ~/.phantom/ai/aliases on the selected session or beacon. Use search_aliases first when you need command metadata or a root_path to disambiguate duplicates.",
			Parameters: map[string]any{
				"type": "object",
				"properties": mergeSchemaProperties(map[string]any{
		...
		},
		{
			...
			Description: "Execute a server-side AI extension from ~/.phantom/ai/extensions on the selected session or beacon. BOFs are supported and dependencies such as coff-loader are registered automatically when present in the server-side store.",
			Parameters: map[string]any{
				"type": "object",
				"properties": mergeSchemaProperties(map[string]any{
...
func (e *executor) callExecuteAlias(ctx context.Context, args executeAliasArgs) (string, error) {
	...
	case manifest.IsAssembly:
		...
		resp, err := callTargetRPC(
			ctx,
			target,
			func(callCtx context.Context, req *commonpb.Request) (*phantompb.ExecuteAssembly, error) {
				return e.backend.ExecuteAssembly(callCtx, &phantompb.ExecuteAssemblyReq{
					Request:     req,
					IsDLL:       isDLL,
					Process:     processName,
					Arguments:   execArgs,
			...
			func() *phantompb.ExecuteAssembly { return &phantompb.ExecuteAssembly{} },
		)
		if err != nil {
			return "", err
		}
	...
	case manifest.IsReflective:
		resp, err := callTargetRPC(
			ctx,
			target,
			func(callCtx context.Context, req *commonpb.Request) (*phantompb.SpawnDll, error) {
				return e.backend.SpawnDll(callCtx, &phantompb.InvokeSpawnDllReq{
					Request:     req,
					Args:        execArgs,
					Data:        binData,
			...
			func() *phantompb.SpawnDll { return &phantompb.SpawnDll{} },
		)
		if err != nil {
			return "", err
		}
	...
	default:
		resp, err := callTargetRPC(
			ctx,
			target,
			func(callCtx context.Context, req *commonpb.Request) (*phantompb.Sideload, error) {
				return e.backend.Sideload(callCtx, &phantompb.SideloadReq{
					Request:     req,
					Data:        binData,
					ProcessName: processName,
					Args:        execArgs,
			...
			func() *phantompb.Sideload { return &phantompb.Sideload{} },
		)
		if err != nil {
			return "", err
		}
...
func (e *executor) callExecuteExtension(ctx context.Context, args executeExtensionArgs) (string, error) {
	...
	loaded, err := callTargetRPC(
		ctx,
		target,
		func(callCtx context.Context, req *commonpb.Request) (*phantompb.ListExtensions, error) {
			return e.backend.ListExtensions(callCtx, &phantompb.ListExtensionsReq{Request: req})
		},
		func() *phantompb.ListExtensions { return &phantompb.ListExtensions{} },
	)
	if err != nil {
		return "", err
	}
	...
	resp, err := callTargetRPC(
		ctx,
		target,
		func(callCtx context.Context, req *commonpb.Request) (*phantompb.CallExtension, error) {
			return e.backend.CallExtension(callCtx, &phantompb.CallExtensionReq{
				Request: req,
				Name:    callName,
				Export:  callExport,
				Args:    callArgs,
		...
		func() *phantompb.CallExtension { return &phantompb.CallExtension{} },
	)
	if err != nil {
		return "", err
	}
...
func (e *executor) ensureExtensionRegistered(ctx context.Context, target toolTarget, loaded []string, command *aiExtensionCommand, artifactPath string, artifactData []byte, targetOS string) (string, error) {
	...
	_, err := callTargetRPC(
		ctx,
		target,
		func(callCtx context.Context, req *commonpb.Request) (*phantompb.RegisterExtension, error) {
			return e.backend.RegisterExtension(callCtx, &phantompb.RegisterExtensionReq{
				Request: req,
				Name:    registeredName,
				Data:    artifactData,
				OS:      targetOS,
		...
		func() *phantompb.RegisterExtension { return &phantompb.RegisterExtension{} },
	)
	if err != nil {
