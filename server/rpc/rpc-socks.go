...
var (
	// SessionID->Tunnels[TunnelID]->Tunnel->Cache map[uint64]*phantompb.SocksData{}
	toImplantCacheSocks = socksDataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*phantompb.SocksData{}, lastActivity: map[uint64]time.Time{}}

	// SessionID->Tunnels[TunnelID]->Tunnel->Cache
	fromImplantCacheSocks = socksDataCache{mutex: &sync.RWMutex{}, cache: map[uint64]map[uint64]*phantompb.SocksData{}, lastActivity: map[uint64]time.Time{}}
)

type socksDataCache struct {
	mutex        *sync.RWMutex
	cache        map[uint64]map[uint64]*phantompb.SocksData
	lastActivity map[uint64]time.Time
}

func (c *socksDataCache) Add(tunnelID uint64, sequence uint64, tunnelData *phantompb.SocksData) {
	c.mutex.Lock()
	defer c.mutex.Unlock()

	if _, ok := c.cache[tunnelID]; !ok {
		c.cache[tunnelID] = map[uint64]*phantompb.SocksData{}
	}

	c.cache[tunnelID][sequence] = tunnelData
}

func (c *socksDataCache) Get(tunnelID uint64, sequence uint64) (*phantompb.SocksData, bool) {
	c.mutex.RLock()
...
func (s *Server) SocksProxy(stream rpcpb.PhantomRPC_SocksProxyServer) error {
	...
	defer func() {
		...
		for tunnelID := range activeTunnels {
			if tunnel := core.SocksTunnels.Get(tunnelID); tunnel != nil {
				...
				tunnel.Client = nil
				s.CloseSocks(context.Background(), &phantompb.Socks{TunnelID: tunnelID})
			}
		}
		tunnelMutex.Unlock()
	}()
	...
	for {
		...
		if tunnel.Client == nil {
			...
			tunnel.FromImplant = make(chan *phantompb.SocksData, 100) // Buffered channel for 100 messages
			...
			go func() {
				...
				pendingData := make(map[uint64]*phantompb.SocksData)
				...
				for {
					...
					case tunnelData, ok := <-tunnel.FromImplant:
						...
						if tunnelData.CloseConn {
							// Clean up the tunnel
							s.CloseSocks(context.Background(), &phantompb.Socks{TunnelID: fromClient.TunnelID})
							return
						}
					...
					case <-ticker.C:
						...
						for i := 0; i < batchSize && processed < len(pendingData); i++ {
							...
							func() {
								...
								err := stream.Send(&phantompb.SocksData{
					...
					case <-ticker.C:
						...
						func() {
							...
							for {
								...
								select {
								case session.Connection.Send <- &phantompb.Envelope{
									Type: phantompb.MsgSocksData,
									Data: data,
								}:
...
func (s *Server) CreateSocks(ctx context.Context, req *phantompb.Socks) (*phantompb.Socks, error) {
...
func (s *Server) CloseSocks(ctx context.Context, req *phantompb.Socks) (*commonpb.Empty, error) {
	defer func() {
		if r := recover(); r != nil {
	...
	if tunnel != nil {
		...
		if session := core.Sessions.Get(tunnel.SessionID); session != nil {
			data, _ := proto.Marshal(&phantompb.SocksData{
				TunnelID:  req.TunnelID,
				CloseConn: true,
			})
			session.Connection.Send <- &phantompb.Envelope{
				Type: phantompb.MsgSocksData,
				Data: data,
			}
		}
