...
func (rpc *Server) KillSession(ctx context.Context, kill *phantompb.KillReq) (*commonpb.Empty, error) {
	...
	session.Request(phantompb.MsgNumber(kill), timeout, data)
	return &commonpb.Empty{}, nil
}

// OpenSession - Instruct beacon to open a new session on next checkin
func (rpc *Server) OpenSession(ctx context.Context, openSession *phantompb.OpenSession) (*phantompb.OpenSession, error) {
	resp := &phantompb.OpenSession{Response: &commonpb.Response{}}
...
func (rpc *Server) CloseSession(ctx context.Context, closeSession *phantompb.CloseSession) (*commonpb.Empty, error) {
	...
	go func() {
		select {
		case session.Connection.Send <- &phantompb.Envelope{Type: phantompb.MsgCloseSession}:
