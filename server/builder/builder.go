...
var (
	builderLog = log.NamedLogger("builder", "phantom")
)
...
func NewBuilder(config *clientpb.Builder, m *sync.Mutex, rpc rpcpb.PhantomRPCClient, ln *grpc.ClientConn) *Builder {
	return &Builder{
...
func (b *Builder) handleBuildEvent(event *clientpb.Event) {
	...
	if extConfig.Config.TemplateName != "phantom" {
	...
	case clientpb.OutputFormat_EXECUTABLE:
		b.mutex.Lock()
		fPath, err = generate.PhantomExecutable(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	case clientpb.OutputFormat_SHARED_LIB:
		b.mutex.Lock()
		fPath, err = generate.PhantomSharedLibrary(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	case clientpb.OutputFormat_SHELLCODE:
		b.mutex.Lock()
		fPath, err = generate.PhantomShellcode(extConfig.Build.Name, extConfig.Build, extConfig.Config, httpC2Config.ImplantConfig)
		b.mutex.Unlock()
	default:
	...
	if err != nil {
		builderLog.Errorf("Failed to generate phantom: %s", err)
		b.rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantBuildID, err.Error())),
		})
	...
	if err != nil {
		builderLog.Errorf("Failed to read generated phantom: %s", err)
		b.rpc.BuilderTrigger(context.Background(), &clientpb.Event{
			EventType: consts.ExternalBuildFailedEvent,
			Data:      []byte(fmt.Sprintf("%s:%s", implantBuildID, err.Error())),
		})
