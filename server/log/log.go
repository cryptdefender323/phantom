...
func GetRootAppDir() string {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if err != nil {
			panic("Cannot write to phantom root dir")
		}
	}
	return dir
}
...
func rootLogger() *logrus.Logger {
	...
	jsonFilePath := filepath.Join(GetLogDir(), "phantom.json")
...
func txtLogger() *logrus.Logger {
	...
	txtFilePath := filepath.Join(GetLogDir(), "phantom.log")
...
func (hook *TxtHook) Fire(entry *logrus.Entry) error {
	...
	srcFile := "<no caller>"
	if entry.HasCaller() {
		phantomIndex := strings.Index(entry.Caller.File, "phantom")
		srcFile = entry.Caller.File
		if phantomIndex != -1 {
			srcFile = srcFile[phantomIndex:]
		}
	}
