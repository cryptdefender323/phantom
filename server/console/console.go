...
import (
	...
	clienttransport "github.com/cryptdefender3232/phantom/client/transport"
...
func Start(rcScript string) {
	...
	defer conn.Close()
	localRPC := rpcpb.NewPhantomRPCClient(conn)
	con := console.NewConsole(true)
	_ = console.StartClient(con, localRPC, conn, nil, command.ServerCommands(con, serverOnlyCmds), command.PhantomCommands(con), true, rcScript)
}
...
remote machines to connect to the Phantom server. They are most commonly
...
new-operator --name <operator name> --lhost <phantom server> --permissions all

Operator profiles can also be used to allow remote machines to connect to
the Phantom server for other purposes, such as a "Remote Builder" or a
"Crackstation."
...
new-operator --name <operator name> --lhost <phantom server> --permissions builder
`
