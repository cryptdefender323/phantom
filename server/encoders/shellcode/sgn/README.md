...
The log subsystem expects a writable Phantom root directory. Point it to a temporary location when running tests:

```bash
export PHANTOM_ROOT_DIR=$(pwd)/.tmp-phantom
export GOCACHE=$(pwd)/.tmp-gocache
mkdir -p "$PHANTOM_ROOT_DIR" "$GOCACHE"
