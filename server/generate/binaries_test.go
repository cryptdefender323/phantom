...
func cleanupGeneratedArtifacts(t *testing.T, outputPath string) {
	...
	phantomsDir := GetPhantomsDir()
	rel, err := filepath.Rel(phantomsDir, projectDir)
...
func TestPhantomExecutableWindows(t *testing.T) {
...
func TestPhantomSharedLibWindows(t *testing.T) {
	// amd64
...
func TestPhantomExecutableLinux(t *testing.T) {
	// amd64
...
func TestPhantomSharedLibraryLinux(t *testing.T) {
	// amd64
...
func TestPhantomExecutableDarwin(t *testing.T) {
	// amd64
...
func TestPhantomDefaultBuild(t *testing.T) {
...
func trafficEncodersExecutable(t *testing.T, goos string, goarch string) {
	...
	nonce++
	binPath, err := PhantomExecutable(name, build, debugConfig, debugHttpC2Config.ImplantConfig)
	if err != nil {
		t.Fatalf("%v", err)
	}
	...
	nonce++
	binPath, err = PhantomExecutable(name, build, prodConfig, debugHttpC2Config.ImplantConfig)
	if err != nil {
		t.Fatalf("%v", err)
	}
	cleanupGeneratedArtifacts(t, binPath)
...
func mtlsExe(t *testing.T, goos string, goarch string, beacon bool, debug bool) {
	...
	nonce++
	binPath, err := PhantomExecutable(name, build, config, httpC2Config.ImplantConfig)
	if err != nil {
		t.Fatalf("%v", err)
	}
	cleanupGeneratedArtifacts(t, binPath)
...
func multiLibrary(t *testing.T, goos string, goarch string, debug bool) {
	...
	nonce++
	build, _ := GenerateConfig(name, config)
	binPath, err := PhantomSharedLibrary(name, build, config, httpC2Config.ImplantConfig)
	if err != nil {
		t.Fatalf("%v", err)
	}
	cleanupGeneratedArtifacts(t, binPath)
