...
import (
	...
	"github.com/cryptdefender3232/phantom/implant"
	"github.com/cryptdefender3232/phantom/protobuf/clientpb"
	"github.com/cryptdefender3232/phantom/server/assets"
	"github.com/cryptdefender3232/phantom/server/certs"
	"github.com/cryptdefender3232/phantom/server/configs"
	"github.com/cryptdefender3232/phantom/server/cryptography"
	"github.com/cryptdefender3232/phantom/server/db/models"
	"github.com/cryptdefender3232/phantom/server/encoders"
	"github.com/cryptdefender3232/phantom/server/gogo"
	"github.com/cryptdefender3232/phantom/server/gogo/goname"
	"github.com/cryptdefender3232/phantom/server/log"
	"github.com/cryptdefender3232/phantom/util"
	utilEncoders "github.com/cryptdefender3232/phantom/util/encoders"
	"github.com/cryptdefender3232/phantom/internal/loaders/dylib"
	"github.com/cryptdefender3232/phantom/internal/loaders/soloader"
...
const (
	...
	// PhantomCC64EnvVar - Environment variable that can specify the 64 bit mingw path
	PhantomCC64EnvVar = "PHANTOM_CC_64"
	// PhantomCC32EnvVar - Environment variable that can specify the 32 bit mingw path
	PhantomCC32EnvVar = "PHANTOM_CC_32"

	// PhantomCXX64EnvVar - Environment variable that can specify the 64 bit mingw path
	PhantomCXX64EnvVar = "PHANTOM_CXX_64"
	// PhantomCXX32EnvVar - Environment variable that can specify the 32 bit mingw path
	PhantomCXX32EnvVar = "PHANTOM_CXX_32"

	// *** Platform Specific ***

	// PhantomPlatformCC64EnvVar - Environment variable that can specify the 64 bit mingw path
	PhantomPlatformCC64EnvVar = "PHANTOM_%s_CC_64"
	// PhantomPlatformCC32EnvVar - Environment variable that can specify the 32 bit mingw path
	PhantomPlatformCC32EnvVar = "PHANTOM_%s_CC_32"
	// PhantomPlatformCXX64EnvVar - Environment variable that can specify the 64 bit mingw path
	PhantomPlatformCXX64EnvVar = "PHANTOM_%s_CXX_64"
	// PhantomPlatformCXX32EnvVar - Environment variable that can specify the 32 bit mingw path
	PhantomPlatformCXX32EnvVar = "PHANTOM_%s_CXX_32"
)
...
func sanitizeZigForBuild(goConfig *gogo.GoConfig, buildmode string) {
	...
	// don't leak the Phantom-managed zig root dir (often under ~/.phantom/zig) into
	// build artifacts.
	...
	// zig's default linux toolchain target in Phantom is musl, which is great for
...
// GetPhantomsDir - Get the binary directory
func GetPhantomsDir() string {
	appDir := assets.GetRootAppDir()
	phantomsDir := filepath.Join(appDir, phantomsDirName)
	if _, err := os.Stat(phantomsDir); os.IsNotExist(err) {
		buildLog.Debugf("Creating bin directory: %s", phantomsDir)
		err = os.MkdirAll(phantomsDir, 0700)
		if err != nil {
			buildLog.Fatal(err)
		}
	}
...
// PhantomShellcode - Generates a phantom shellcode (Windows: Donut, macOS: beignet, Linux: malasada)
func PhantomShellcode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	switch config.GOOS {
	case WINDOWS:
...
func linuxShellcode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	pkgPath, err := renderPhantomGoCode(name, build, config, goConfig, pbC2Implant)
	if err != nil {
		return "", err
	}

	tmpFile, err := os.CreateTemp("", "phantom-*.so")
	if err != nil {
		return "", err
	}
	tmpFile.Close()
	...
	shellcodeBin, err := soloader.ConvertSharedObject(soDest, config.Exports[0], compress)
	if err != nil {
		return "", err
	}
...
func darwinShellcode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	tmpFile, err := os.CreateTemp("", "phantom-*.dylib")
	if err != nil {
		return "", err
	}
	tmpFile.Close()
	...
	shellcodeBin, err := dylib.ToShellcode(dylibData, dylib.Options{
...
// PhantomSharedLibrary - Generates a phantom shared library (DLL/dylib/so) binary
func PhantomSharedLibrary(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	appDir := assets.GetRootAppDir()
...
// PhantomExecutable - Generates a phantom executable binary
func PhantomExecutable(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	appDir := assets.GetRootAppDir()
...
func renderPhantomGoCode(name string, build *clientpb.ImplantBuild, config *clientpb.ImplantConfig, goConfig *gogo.GoConfig, pbC2Implant *clientpb.HTTPC2ImplantConfig) (string, error) {
	...
	phantomsDir := GetPhantomsDir() // ~/.phantom/phantoms
	projectGoPathDir := filepath.Join(phantomsDir, config.GOOS, config.GOARCH, filepath.Base(name))
	if _, err := os.Stat(projectGoPathDir); os.IsNotExist(err) {
		os.MkdirAll(projectGoPathDir, 0700)
	}
	goConfig.ProjectDir = projectGoPathDir

	// binDir - ~/.phantom/phantoms/<os>/<arch>/<name>/bin
	binDir := filepath.Join(projectGoPathDir, "bin")
	os.MkdirAll(binDir, 0700)

	// srcDir - ~/.phantom/phantoms/<os>/<arch>/<name>/src
	...
	phantomPkgDir := filepath.Join(srcDir) // "main"
	err = os.MkdirAll(phantomPkgDir, 0700)
	if err != nil {
		return "", nil
	}
	...
	err = fs.WalkDir(implant.FS, ".", func(fsPath string, f fs.DirEntry, err error) error {
		...
		phantomGoCodeRaw, err := implant.FS.ReadFile(fsPath)
		if err != nil {
			buildLog.Errorf("Failed to read %s: %s", fsPath, err)
			return nil
		}
		phantomGoCode := string(phantomGoCodeRaw)
		...
		var phantomCodePath string
		if f.Name() == "main.go" || f.Name() == "main.c" || f.Name() == "main.h" {
			phantomCodePath = filepath.Join(phantomPkgDir, f.Name())
		} else {
			phantomCodePath = filepath.Join(phantomPkgDir, "implant", fsPath)
		}
		dirPath := filepath.Dir(phantomCodePath)
		...
		fPhantom, err := os.Create(phantomCodePath)
		if err != nil {
			return err
		}
		if !util.Contains([]string{".go", ".c", ".h"}, path.Ext(f.Name())) {
			...
			_, err = fPhantom.Write(phantomGoCodeRaw)
			return err
		}
		...
		buildLog.Debugf("[render] %s -> %s", f.Name(), phantomCodePath)

		phantomCode := template.New("phantom")
		phantomCode, err = phantomCode.Funcs(template.FuncMap{
		...
		err = phantomCode.Execute(buf, struct {
			Name                string
		...
		err = canaryTemplate.Execute(fPhantom, canaryGenerator)
		if err != nil {
	...
	renderNativeEncoderAssets(build, config, phantomPkgDir)
	if config.TrafficEncodersEnabled {
		renderTrafficEncoderAssets(build, config, phantomPkgDir)
	}

	// Render GoMod
	buildLog.Info("Rendering go.mod file ...")
	goModPath := filepath.Join(phantomPkgDir, "go.mod")
	err = os.WriteFile(goModPath, []byte(implant.GoMod), 0600)
	if err != nil {
		return "", err
	}
	goSumPath := filepath.Join(phantomPkgDir, "go.sum")
	...
	err = fs.WalkDir(implant.Vendor, ".", func(path string, d fs.DirEntry, err error) error {
		...
		if d.IsDir() {
			return os.MkdirAll(filepath.Join(phantomPkgDir, path), 0700)
		}
		...
		return os.WriteFile(filepath.Join(phantomPkgDir, path), contents, 0600)
	})
	if err != nil {
	...
	if !config.Debug {
		...
		modResult, err := goname.RenameModule(phantomPkgDir, goPackage)
		if err != nil {
		...
		oldImportPrefix := path.Join(modResult.NewModule, "implant", "phantom")
		newImportPrefix := path.Join(modResult.NewModule, "runc", "cgroup")
		importResult, err := goname.RenameImport(phantomPkgDir, oldImportPrefix, newImportPrefix)
		if err != nil {
	...
	return phantomPkgDir, nil
}

// renderTrafficEncoderAssets - Copies and compresses any enabled WASM traffic encoders
func renderTrafficEncoderAssets(_ *clientpb.ImplantBuild, config *clientpb.ImplantConfig, phantomPkgDir string) {
	buildLog.Infof("Rendering traffic encoder assets ...")
	encoderAssetsPath := filepath.Join(phantomPkgDir, "implant", "phantom", "encoders", "assets")
...
func renderNativeEncoderAssets(_ *clientpb.ImplantBuild, _ *clientpb.ImplantConfig, phantomPkgDir string) {
...
func GenerateConfig(name string, implantConfig *clientpb.ImplantConfig) (*clientpb.ImplantBuild, error) {
	...
	phantomCert, phantomKey, err := certs.MtlsC2ImplantGenerateECCCertificate(name)
	if err != nil {
		return nil, err
	}
	...
	if models.IsC2Enabled([]string{"mtls"}, implantConfig.C2) {
		...
		build.MtlsCert = string(phantomCert)
		build.MtlsKey = string(phantomKey)
	}
...
func getCrossCompilersFromEnv(targetGoos string, targetGoarch string) (string, string) {
	...
	if targetGoarch == "amd64" {
		if os.Getenv(fmt.Sprintf(PhantomPlatformCC64EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(PhantomPlatformCC64EnvVar, TARGET_GOOS))
		}
		if cc == "" {
			cc = os.Getenv(PhantomCC64EnvVar)
		}
		if os.Getenv(fmt.Sprintf(PhantomPlatformCXX64EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(PhantomPlatformCXX64EnvVar, TARGET_GOOS))
		}
		if cxx == "" {
			cxx = os.Getenv(PhantomCXX64EnvVar)
		}
	}
	if targetGoarch == "386" {
		cc = os.Getenv(PhantomCC32EnvVar)
		if os.Getenv(fmt.Sprintf(PhantomPlatformCC32EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(PhantomPlatformCC32EnvVar, TARGET_GOOS))
		}
		cxx = os.Getenv(PhantomCXX64EnvVar)
		if os.Getenv(fmt.Sprintf(PhantomPlatformCXX32EnvVar, TARGET_GOOS)) != "" {
			cc = os.Getenv(fmt.Sprintf(PhantomPlatformCXX32EnvVar, TARGET_GOOS))
		}
	}
	return cc, cxx
}
...
func findCrossCompilers(targetGOOS string, targetGOARCH string) (string, string) {
	...
	// paths under the Phantom zig dir from leaking into artifacts).
