...
// Tunnel  - Essentially just a mapping between a specific client and phantom
...
type Tunnel struct {
	...
	FromImplant         chan *phantompb.TunnelData
	FromImplantSequence uint64

	Client rpcpb.PhantomRPC_TunnelDataServer
...
func NewTunnel(id uint64, sessionID string) *Tunnel {
	return &Tunnel{
		...
		FromImplant: make(chan *phantompb.TunnelData),
...
func (t *Tunnel) SendDataFromImplant(tunnelData *phantompb.TunnelData) {
...
func (t *tunnels) Close(tunnelID uint64) error {
	...
	tunnelClose, err := proto.Marshal(&phantompb.TunnelData{
		TunnelID:  tunnel.ID,
	...
	data, err := proto.Marshal(&phantompb.Envelope{
