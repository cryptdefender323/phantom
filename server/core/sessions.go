...
// Request - Sends a protobuf request to the active phantom and returns the response
func (s *Session) Request(msgType uint32, timeout time.Duration, data []byte) ([]byte, error) {
	resp := make(chan *phantompb.Envelope)
	reqID := EnvelopeID()
	...
	select {
	case s.Connection.Send <- &phantompb.Envelope{
		ID:   reqID,
		Type: msgType,
		Data: data,
	}:
	...
	var respEnvelope *phantompb.Envelope
	select {
...
// sessions - Manages the phantoms, provides atomic access
...
// Add - Add a phantom to the hive (atomically)
...
// Remove - Remove a phantom from the hive (atomically)
