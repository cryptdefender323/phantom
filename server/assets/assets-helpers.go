...
import (
	...
	protobufs "github.com/cryptdefender3232/phantom/protobuf"
...
func SetupGoPath(goPathSrc string, includeDNS bool) error {
	...
	// Phantom PB
	phantompbGoSrc, err := protobufs.FS.ReadFile("phantompb/phantom.pb.go")
	if err != nil {
		setupLog.Info("Static asset not found: phantom.pb.go")
		return err
	}
	phantompbConstSrc, err := protobufs.FS.ReadFile("phantompb/constants.go")
	if err != nil {
		setupLog.Info("Static asset not found: constants.go")
		return err
	}
	phantompbGoSrc = xorPBRawBytes(phantompbGoSrc)
	phantompbGoSrc = stripPhantompb(phantompbGoSrc)
	phantompbDir := filepath.Join(goPathSrc, "protobuf", "phantompb")
	setupLog.Infof("Creating phantompb directory: %s", phantompbDir)
	os.MkdirAll(phantompbDir, 0700)
	os.WriteFile(filepath.Join(phantompbDir, "phantom.pb.go"), phantompbGoSrc, 0600)
	os.WriteFile(filepath.Join(phantompbDir, "constants.go"), phantompbConstSrc, 0600)
...
func stripPhantompb(src []byte) []byte {
	out := src
