...
import (
	...
	ver "github.com/cryptdefender3232/phantom/server/version"
)
...
// GetRootAppDir - Get the Phantom app dir, default is: ~/.phantom/
func GetRootAppDir() string {
	value := os.Getenv(envVarName)
	var dir string
	if len(value) == 0 {
		user, _ := user.Current()
		dir = filepath.Join(user.HomeDir, ".phantom")
	} else {
		dir = value
	}
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		...
		if err != nil {
			setupLog.Fatalf("Cannot write to phantom root dir %s", err)
		}
	}
	return dir
}
...
// GetChunkDataDir - Get the Phantom chunk data dir, default is: ~/.phantom/crack/chunks/
...
// GetTrafficEncoderDir - Get the Phantom traffic encoder dir, default is: ~/.phantom/traffic-encoders/
