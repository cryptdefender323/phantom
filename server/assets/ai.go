...
const (
	// AIDirName stores server-side AI helper data under the Phantom root.
	AIDirName = "ai"
...
// GetAIDir returns the server-side AI data directory: ~/.phantom/ai
func GetAIDir() string {
	return ensureAIDir(filepath.Join(GetRootAppDir(), AIDirName))
}

// GetAIAliasesDir returns the server-side AI alias directory: ~/.phantom/ai/aliases
func GetAIAliasesDir() string {
	return ensureAIDir(filepath.Join(GetAIDir(), AIAliasesDirName))
}

// GetAIExtensionsDir returns the server-side AI extension directory: ~/.phantom/ai/extensions
