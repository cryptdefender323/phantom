...
func StartMutualTLSListener(bindIface string, port uint16) (net.Listener, error) {
	...
	go acceptPhantomConnections(ln)
	return ln, nil
}

func acceptPhantomConnections(ln net.Listener) {
	defer recoverAndLogPanic(mtlsLog.Errorf, "mtls acceptPhantomConnections")
	...
	for {
		...
		go handlePhantomConnection(conn)
	}
}

func handlePhantomConnection(conn net.Conn) {
	defer recoverAndLogPanic(mtlsLog.Errorf, "mtls handlePhantomConnection")
	...
	if err == nil && bytes.Equal(preface, mtlsYamuxPrefaceBytes) {
		...
		handlePhantomConnectionYamux(bufferedConn, implantConn)
		return
	}
...
func handlePhantomConnectionYamux(conn net.Conn, implantConn *core.ImplantConnection) {
	defer recoverAndLogPanic(mtlsLog.Errorf, "mtls handlePhantomConnectionYamux")
...
func socketWriteEnvelope(connection net.Conn, envelope *phantompb.Envelope) error {
...
func socketReadEnvelope(connection net.Conn) (*phantompb.Envelope, error) {
