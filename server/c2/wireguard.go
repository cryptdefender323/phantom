...
import (
	...
	serverCrypto "github.com/cryptdefender3232/phantom/server/cryptography"
	"github.com/cryptdefender3232/phantom/server/generate"
	serverHandlers "github.com/cryptdefender3232/phantom/server/handlers"
	"github.com/cryptdefender3232/phantom/server/log"
	"github.com/cryptdefender3232/phantom/server/netstack"
	"github.com/cryptdefender3232/phantom/util/minisign"
...
func StartWGListener(port uint16, netstackPort uint16, keyExchangeListenPort uint16) (net.Listener, *device.Device, *bytes.Buffer, error) {
	...
	if err != nil {
		wgLog.Errorf("Failed to setup up wg phantom listener: %v", err)
		return nil, nil, nil, err
	}
	wgLog.Printf("Successfully setup up wg phantom listener")
	go acceptWGPhantomConnections(listener)
...
func acceptWGPhantomConnections(ln net.Listener) {
	defer recoverAndLogPanic(wgLog.Errorf, "wireguard acceptWGPhantomConnections")
	...
	for {
		...
		go handleWGPhantomConnection(conn)
	}
}

func handleWGPhantomConnection(conn net.Conn) {
	defer recoverAndLogPanic(wgLog.Errorf, "wireguard handleWGPhantomConnection")
	...
	if err == nil && bytes.Equal(preface, wgYamuxPrefaceBytes) {
		...
		handleWGPhantomConnectionYamux(bufferedConn, implantConn)
		return
	}
...
func handleWGPhantomConnectionYamux(conn net.Conn, implantConn *core.ImplantConnection) {
	defer recoverAndLogPanic(wgLog.Errorf, "wireguard handleWGPhantomConnectionYamux")
		...
		for {
			...
			go func(stream net.Conn) {
				...
				if handler, ok := handlers[envelope.Type]; ok {
					go func(envelope *phantompb.Envelope) {
...
func socketWGWriteEnvelope(connection net.Conn, envelope *phantompb.Envelope) error {
...
func socketWGReadEnvelope(connection net.Conn) (*phantompb.Envelope, error) {
