...
import (
	...
	implantEncoders "github.com/cryptdefender3232/phantom/implant/phantom/encoders"
...
func randomDNSMsgs(t *testing.T, parent string, maxSize int, encoder encoders.Encoder, client *dnsclient.PhantomDNSClient) ([]*dnspb.DNSMessage, []byte) {
...
func TestDNSWriteEnvelopeRoundTrip(t *testing.T) {
	...
	respCh := make(chan *phantompb.Envelope, 1)
...
func newTestDNSServer() *PhantomDNSServer {
	return &PhantomDNSServer{
		sessions:     &sync.Map{},
		messages:     &sync.Map{},
		TTL:          0,
...
func sendEnvelopeViaDNS(t *testing.T, resolver *testDNSResolver, sessionID uint32, key [32]byte, envelope *phantompb.Envelope) {
	t.Helper()
...
func readEnvelopeViaDNS(t *testing.T, resolver *testDNSResolver, sessionID uint32, key [32]byte, noTXT bool) (*phantompb.Envelope, error) {
	t.Helper()
	pollMsg := &dnspb.DNSMessage{
		Type: dnspb.DNSMessageType_POLL,
		ID:   sessionID,
