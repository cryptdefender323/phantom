...
import (
	...
	implantCrypto "github.com/cryptdefender3232/phantom/implant/phantom/cryptography"
	implantHandlers "github.com/cryptdefender3232/phantom/implant/phantom/handlers"
	"github.com/cryptdefender3232/phantom/implant/phantom/transports/dnsclient"
...
func startTestDNSImplant(t *testing.T, dnsParent string, dnsPort uint32) func() {
	...
	var (
		client *dnsclient.PhantomDNSClient
		err    error
	)
	...
	if err := client.WriteEnvelope(&phantompb.Envelope{Type: phantompb.MsgRegister, Data: regData}); err != nil {
	...
	go func() {
		...
		for {
			...
			if envelope.Type == dnsE2EEchoMsgType {
				_ = client.WriteEnvelope(&phantompb.Envelope{ID: envelope.ID, Data: envelope.Data})
				continue
			}

			handler, ok := handlers[envelope.Type]
			if !ok {
				_ = client.WriteEnvelope(&phantompb.Envelope{ID: envelope.ID, UnknownMessageType: true})
				continue
			}

			handler(envelope.Data, func(data []byte, err error) {
				_ = err
				_ = client.WriteEnvelope(&phantompb.Envelope{ID: envelope.ID, Data: data})
			})
		}
	}()
