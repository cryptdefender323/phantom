...
func startTestImplant(t *testing.T, conn net.Conn) func() {
	...
	if err := sendYamuxEnvelope(muxSession, &phantompb.Envelope{Type: phantompb.MsgRegister, Data: regData}); err != nil {
	...
	go func() {
		...
		for {
			...
			go func(stream net.Conn) {
				...
				if !ok {
					_ = sendYamuxEnvelope(muxSession, &phantompb.Envelope{ID: envelope.ID, UnknownMessageType: true})
					return
				}
				handler(envelope.Data, func(data []byte, err error) {
					_ = err
					_ = sendYamuxEnvelope(muxSession, &phantompb.Envelope{ID: envelope.ID, Data: data})
				})
			}(stream)
		}
	}()
...
func sendYamuxEnvelope(session *yamux.Session, envelope *phantompb.Envelope) error {
