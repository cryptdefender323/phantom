...
import (
	...
	implantWG "github.com/cryptdefender3232/phantom/implant/phantom/transports/wireguard"
...
func TestWGYamux_Beacon_EndToEndAsyncPingRPC(t *testing.T) {
	...
	go c2.HandleWGPhantomConnectionForTest(serverConn)
...
func startTestWGBeacon(t *testing.T, conn net.Conn, beaconID string) *wgTestBeacon {
	...
	if err := sendWGYamuxEnvelope(muxSession, &phantompb.Envelope{Type: phantompb.MsgBeaconRegister, Data: regData}); err != nil {
...
func (b *wgTestBeacon) FetchPendingTasks(ctx context.Context) ([]*phantompb.Envelope, error) {
	...
	if err := sendWGYamuxEnvelope(b.session, &phantompb.Envelope{Type: phantompb.MsgBeaconTasks, Data: reqData}); err != nil {
		return nil, err
	}
...
func (b *wgTestBeacon) SendResults(results []*phantompb.Envelope) error {
	reqData, err := proto.Marshal(&phantompb.BeaconTasks{
		ID:          b.id,
		NextCheckin: int64(1),
		Tasks:       results,
	...
	return sendWGYamuxEnvelope(b.session, &phantompb.Envelope{Type: phantompb.MsgBeaconTasks, Data: reqData})
}
