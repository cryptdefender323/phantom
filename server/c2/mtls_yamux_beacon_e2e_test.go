...
import (
	...
	implantHandlers "github.com/cryptdefender3232/phantom/implant/phantom/handlers"
	implantMTLS "github.com/cryptdefender3232/phantom/implant/phantom/transports/mtls"
	...
	"github.com/cryptdefender3232/phantom/server/c2"
	"github.com/cryptdefender3232/phantom/server/db/models"
	"github.com/cryptdefender3232/phantom/server/transport"
...
func TestMTLSYamux_Beacon_EndToEndAsyncPingRPC(t *testing.T) {
	...
	// `~/.phantom` are blocked, set `PHANTOM_ROOT_DIR` to a writable temp dir.
	...
	go c2.HandlePhantomConnectionForTest(serverConn)
	...
	rpcClient := rpcpb.NewPhantomRPCClient(rpcConn)
	...
	for i := 0; i < tasksCount; i++ {
		...
		nonce := int32(1000 + i)
		resp, err := rpcClient.Ping(callCtx, &phantompb.Ping{
			Nonce: nonce,
...
func TestMTLSYamux_Beacon_EndToEndAsyncMixedRPCs(t *testing.T) {
	...
	{
		callCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		resp, err := rpcClient.Pwd(callCtx, &phantompb.PwdReq{
			Request: &commonpb.Request{
				Async:    true,
				BeaconID: beaconID,
				Timeout:  reqTimeout,
	...
	{
		callCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		resp, err := rpcClient.GetEnv(callCtx, &phantompb.EnvReq{
			Name: "PATH",
			Request: &commonpb.Request{
				Async:    true,
				BeaconID: beaconID,
	...
	{
		callCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
		resp, err := rpcClient.Ls(callCtx, &phantompb.LsReq{
			Path: testDir,
			Request: &commonpb.Request{
				Async:    true,
				BeaconID: beaconID,
...
type testBeacon struct {
	...
	incoming chan *phantompb.Envelope
	done     chan struct{}
}
...
func startTestBeacon(t *testing.T, conn net.Conn, beaconID string) *testBeacon {
	...
	beacon := &testBeacon{
		...
		conn:     conn,
		session:  muxSession,
		incoming: make(chan *phantompb.Envelope, 64),
		done:     make(chan struct{}),
	}
	go beacon.recvLoop()

	register := &phantompb.Register{
		Name:              "e2e-beacon",
		Hostname:          "localhost",
		Uuid:              uuid.NewString(),
		Username:          "unit-test",
	...
	regData, err := proto.Marshal(&phantompb.BeaconRegister{
		ID:          beaconID,
		Interval:    int64(1),
		Jitter:      int64(0),
		Register:    register,
	...
	if err := sendYamuxEnvelope(muxSession, &phantompb.Envelope{Type: phantompb.MsgBeaconRegister, Data: regData}); err != nil {
...
func (b *testBeacon) FetchPendingTasks(ctx context.Context) ([]*phantompb.Envelope, error) {
	reqData, err := proto.Marshal(&phantompb.BeaconTasks{
		ID:          b.id,
		NextCheckin: int64(1),
	})
	if err != nil {
	...
	if err := sendYamuxEnvelope(b.session, &phantompb.Envelope{Type: phantompb.MsgBeaconTasks, Data: reqData}); err != nil {
		return nil, err
	}
	...
	for {
		...
		case envelope, ok := <-b.incoming:
			...
			if envelope.Type != phantompb.MsgBeaconTasks {
				continue
			}
			resp := &phantompb.BeaconTasks{}
...
func (b *testBeacon) SendResults(results []*phantompb.Envelope) error {
	reqData, err := proto.Marshal(&phantompb.BeaconTasks{
		ID:          b.id,
		NextCheckin: int64(1),
		Tasks:       results,
	...
	return sendYamuxEnvelope(b.session, &phantompb.Envelope{Type: phantompb.MsgBeaconTasks, Data: reqData})
}

func executeBeaconTasks(t *testing.T, tasks []*phantompb.Envelope) []*phantompb.Envelope {
	t.Helper()

	sysHandlers := implantHandlers.GetSystemHandlers()
	results := make([]*phantompb.Envelope, 0, len(tasks))
	...
	for _, task := range tasks {
		...
		if !ok {
			results = append(results, &phantompb.Envelope{ID: task.ID, UnknownMessageType: true})
			continue
		}
		...
		case <-done:
			_ = respErr
			results = append(results, &phantompb.Envelope{ID: task.ID, Data: respData})
...
func waitForBeaconRegistration(t *testing.T, rpcClient rpcpb.PhantomRPCClient, beaconID string, timeout time.Duration) {
	t.Helper()
...
func waitForBeaconTaskResponseBytes(t *testing.T, rpcClient rpcpb.PhantomRPCClient, taskID string, timeout time.Duration) []byte {
	t.Helper()
...
func waitForBeaconTaskCompleted(t *testing.T, rpcClient rpcpb.PhantomRPCClient, taskID string, wantNonce int32, timeout time.Duration) {
	t.Helper()
