...
func TestWGYamux_EndToEndMixedRPCs(t *testing.T) {
	...
	pwdResp, err := rpcClient.Pwd(ctx, &phantompb.PwdReq{
		Request: &commonpb.Request{
			SessionID: sessionID,
			Timeout:   reqTimeout,
		},
	...
	envResp, err := rpcClient.GetEnv(ctx, &phantompb.EnvReq{
		Name: "PATH",
		Request: &commonpb.Request{
			SessionID: sessionID,
			Timeout:   reqTimeout,
	...
	lsResp, err := rpcClient.Ls(ctx, &phantompb.LsReq{
		Path: testDir,
		Request: &commonpb.Request{
			SessionID: sessionID,
			Timeout:   reqTimeout,
...
func startTestWGImplant(t *testing.T, conn net.Conn) func() {
	...
	if err := sendWGYamuxEnvelope(muxSession, &phantompb.Envelope{Type: phantompb.MsgRegister, Data: regData}); err != nil {
	...
	go func() {
		...
		for {
			...
			go func(stream net.Conn) {
				...
				if !ok {
					_ = sendWGYamuxEnvelope(muxSession, &phantompb.Envelope{ID: envelope.ID, UnknownMessageType: true})
					return
				}
				handler(envelope.Data, func(data []byte, err error) {
					_ = err
					_ = sendWGYamuxEnvelope(muxSession, &phantompb.Envelope{ID: envelope.ID, Data: data})
				})
			}(stream)
		}
	}()
...
func sendWGYamuxEnvelope(session *yamux.Session, envelope *phantompb.Envelope) error {
