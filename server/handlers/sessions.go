...
import (
	...
	"github.com/cryptdefender3232/phantom/server/core/rtunnels"
...
func registerSessionHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	if implantConn == nil {
		return nil
	}
...
func auditLogSession(session *core.Session, register *phantompb.Register) {
...
func tunnelDataHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	...
	tunnelData := &phantompb.TunnelData{}
...
func tunnelCloseHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func pingHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func socksDataHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)
	if session == nil {
...
func createReverseTunnelHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	session := core.Sessions.FromImplantConnection(implantConn)

	req := &phantompb.TunnelData{}
	...
	if err != nil {
		tunnelClose, _ := proto.Marshal(&phantompb.TunnelData{
			Closed:   true,
			TunnelID: req.TunnelID,
		})
		implantConn.Send <- &phantompb.Envelope{
			Type: phantompb.MsgTunnelClose,
			Data: tunnelClose,
		}
		cancelContext()
		return nil
	...
	if tunnelDataCache.Len(tunnel.ID) > 3 {
		data, err := proto.Marshal(&phantompb.TunnelData{
...
func RTunnelDataHandler(tunnelData *phantompb.TunnelData, tunnel *rtunnels.RTunnel, connection *core.ImplantConnection) {
