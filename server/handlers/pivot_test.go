...
func addPivotFailureTestPivotSession(t *testing.T, originID int64, chain ...int64) (*core.Session, *core.Pivot) {
	...
	peers := make([]*phantompb.PivotPeer, 0, len(chain))
	for _, peerID := range chain {
		peers = append(peers, &phantompb.PivotPeer{
			PeerID: peerID,
...
func mustMarshalPivotPeerFailure(t *testing.T, peerID int64) []byte {
	...
	data, err := proto.Marshal(&phantompb.PivotPeerFailure{
		PeerID: peerID,
		Type:   phantompb.PeerFailureType_SEND_FAILURE,
	})
	if err != nil {
