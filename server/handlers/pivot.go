...
func pivotPeerEnvelopeHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	pivotLog.Debugf("received pivot peer envelope ...")
	peerEnvelope := &phantompb.PivotPeerEnvelope{}
	...
	var resp *phantompb.Envelope
	switch peerEnvelope.Type {
	case phantompb.MsgPivotServerKeyExchange:
		resp = serverKeyExchange(implantConn, peerEnvelope)
	case phantompb.MsgPivotSessionEnvelope:
...
func sessionEnvelopeHandler(implantConn *core.ImplantConnection, peerEnvelope *phantompb.PivotPeerEnvelope) *phantompb.Envelope {
...
func handlePivotEnvelope(pivot *core.Pivot, envelope *phantompb.Envelope) {
	...
	} else if envelope.Type == phantompb.MsgPivotServerPing {
...
func pivotPeerFailureHandler(implantConn *core.ImplantConnection, data []byte) *phantompb.Envelope {
	peerFailure := &phantompb.PivotPeerFailure{}
...
func pivotPeerIndex(peers []*phantompb.PivotPeer, peerID int64) int {
...
// ------------------------
func serverKeyExchange(implantConn *core.ImplantConnection, peerEnvelope *phantompb.PivotPeerEnvelope) *phantompb.Envelope {
	...
	serverKeyEx := &phantompb.PivotServerKeyExchange{}
	...
	keyExRespEnvelope := MustMarshal(&phantompb.Envelope{
		Type: phantompb.MsgPivotServerKeyExchange,
		Data: MustMarshal(&phantompb.PivotServerKeyExchange{
	...
	peerEnvelopeData, _ := proto.Marshal(&phantompb.PivotPeerEnvelope{
	...
	return &phantompb.Envelope{
		Type: phantompb.MsgPivotPeerEnvelope,
...
func peersToString(remoteAddr string, peerEnvelope *phantompb.PivotPeerEnvelope) string {
