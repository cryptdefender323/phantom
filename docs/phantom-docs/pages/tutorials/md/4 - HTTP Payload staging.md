When using Phantom during a live engagement, you’re going to need to use custom stagers, which are essentially a first binary or commandline that will retrieve and/or load Phantom into memory on your target system. Phantom can generate shellcode for your stager to execute by using the `profiles` command.
...
```
https://phantom-ip/definitely/not/malware?x=yourID
```
...
Phantom staging also supports encoding or encrypting our payloads before exposing them externally using the `profile stage` command, the implant configuration remains the same but you are now able to stage different versions of it simultaneously.
