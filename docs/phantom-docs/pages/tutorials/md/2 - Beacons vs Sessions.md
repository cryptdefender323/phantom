Phantom implants support two types of connections, sessions and beacons.
...
```bash
[server] phantom (TIRED_GIRAFFE) > reconfig -i 5s -j 1s
...
[server] phantom (TIRED_GIRAFFE) > info
...
[server] phantom (TIRED_GIRAFFE) > use
...
[server] phantom (TIRED_GIRAFFE) > socks5 start
