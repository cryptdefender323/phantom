Reactions are a basic way to automate tasks in the phantom console, they allow you to specify phantom commands to run on a list of events.
...
However, there are a couple of limitations to keep in mind when using reactions, first off, these are run in the console you are currently using, which is not necessarily the server console. So if you are connected to a phantom server using the phantom client, if you disconnect the client the reactions are no longer running. 
...
First, install the phantom-py extension using pip.

```bash
pip3 install phantom-py
```

Since our extension is essentially going to be another client connection to the phantom server, you’ll also need to enable multiplayer mode and generate a new profile.

Current Phantom releases default to a WireGuard-protected multiplayer listener. Because this tutorial uses a third-party Python client, we’ll force direct multiplayer mode so the profile only contains the plain gRPC/mTLS settings:

```bash
[server] phantom > multiplayer --disable-wg

[*] Multiplayer mode enabled!

[server] phantom > new-operator -n tester -l 127.0.0.1 --permissions all --disable-wg
...
We first need to import a few dependencies, `PhantomClientConfig`, which is used to parse the client config we’ve just created, and `PhantomClient`, which will handle the connection to the backend server.
...
In [1]: from phantom import PhantomClientConfig, PhantomClient

In [2]: DEFAULT_CONFIG = "/Users/tester/tools/tester_127.0.0.1.cfg"

In [3]: config = PhantomClientConfig.parse_config_file(DEFAULT_CONFIG)

In [4]: client = PhantomClient(config)
...
Now that we’ve got the basics of connecting to phantom and running commands down let’s write a more useful script that will display the hosts file when a new session checks in. Our goal is to first identify the Operating System, and then based on that retrieve and display the contents of the hosts file if it exists. Because this script will wait and react to events emitted by the Phantom server, we’re going to use `asyncio` to write our client.
...
import os
import asyncio
from phantom import PhantomClientConfig, PhantomClient
import gzip
...
async def main():
    ...
    config = PhantomClientConfig.parse_config_file(DEFAULT_CONFIG)
    client = PhantomClient(config)
