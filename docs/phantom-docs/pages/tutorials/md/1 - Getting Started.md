`phantom-server` is the binary you want to use to run the Phantom C2 server, `phantom-client` is solely a client to connect to a Phantom C2 server. Phantom server also acts as a client on its own, so you don’t necessarily run phantom server and client separately.

First time running Phantom will take a couple seconds as it's retrieving its dependencies. Consecutive executions will be much faster. Go ahead and launch the `phantom-server`.

```asciinema
{"src": "/asciinema/startup.cast", "cols": "132", "rows": "28", "idleTimeLimit": 8}
```

Let's take a couple minutes to discuss what Phantom actually is and how it's set up.

![Alt text](/images/Architecture.png)

Now that Phantom is running, lets generate and execute your first implant to try out some of the basic features of Phantom, for now we’re going to run everything on the local host.
...
[server] phantom (RELATED_EARDRUM) > pwd
