When generating implants Phantom uses a C2Profile configuration to define how implant callbacks will look over HTTP/s. This allowed operators to control headers, cookies proxies, URL paths and so on. When the implant is generated the phantom server will then select portions of that configuration to generate the implant C2Profile.
