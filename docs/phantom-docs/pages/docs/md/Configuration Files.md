...
The Phantom server configuration file is located in the `configs` sub-directory of [`PHANTOM_ROOT_DIR`](/docs?name=Environment+Variables); by default this is `~/.phantom/configs/server.yaml`.

If no configuration file exists, a default YAML configuration is generated and written to disk on startup. If a legacy `server.json` file exists, Phantom reads it, writes `server.yaml`, and renames the old file to `.server.json-old`.
...
- `daemon` - Options for daemon mode.
  ...
  - `tailscale` - Enable Tailscale for daemon listener setup. When enabled, Phantom does not wrap multiplayer in its own WireGuard layer.
...
- `logs` - Server logging options.
  - `level` - `logrus` level (`0`-`6`, clamped by Phantom). `4` is `INFO`, `5` is `DEBUG`, `6` is `TRACE`.
...
Phantom supports SQL database configurations in `~/.phantom/configs/database.yaml`.

If no configuration file exists, a default YAML configuration is generated and written to disk on startup. If a legacy `database.json` exists, Phantom reads it, writes `database.yaml`, and renames the old file to `.database.json-old`.
...
With the default SQLite configuration, Phantom stores the DB at `~/.phantom/phantom.db`.

# Client Configuration

### Operator

Operator client configs are generated by the server and should be copied into `~/.phantom-client/configs/`. These files are currently JSON (`*.cfg`), not YAML.
...
- When present, `phantom-client` automatically brings up the multiplayer WireGuard wrapper and then dials the in-tunnel gRPC/mTLS service.
- When absent, the client connects directly to the multiplayer mTLS listener.
- Generate a WireGuard-enabled profile with `new-operator --enable-wg` or `phantom-server operator --enable-wg` when you want the multiplayer WireGuard wrapper.
