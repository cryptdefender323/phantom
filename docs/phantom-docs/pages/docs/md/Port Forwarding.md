Phantom provides two mechanisms for port forwarding to tunnel additional connections / tools into the target environment via an implant:

- `portfwd` - This command (available on all C2s) uses Phantom's in-band tunnels to transfer data between your local machine and the implant network (i.e., if you're using HTTP C2 all port forwarded traffic is tunneled over HTTP, same for mTLS/DNS/etc.)
- `wg-portfwd` - This command uses WireGuard port forwarding, and is only available when using WireGuard C2.

**NOTE:** Generally speaking `wg-portfwd` is faster and more reliable, we recommend using it whenever possible. Some protocols may be unstable, or may not work when tunneled via `portfwd`. However, `wg-portfwd` does require a little extra setup (see below).
...
```
phantom (STUCK_ARTICLE) > portfwd add --remote 10.10.10.10:22
...
Phantom also supports reverse port forwarding via the `rportfwd` command.
...
Next, using Phantom you can create a WireGuard client configuration using the `wg-config` command (you can use `--save` to write the configuration directly to a file):
...
The only thing in the configuration file you'll need to change is the `Endpoint` setting, configure this to point to the Phantom server's WireGuard listener, and ensure to include the port number (by default UDP 53). Generally this will be the same value you specified as `--lhost` when generating the binary.
...
Now that your machine is connected to the Phantom WireGuard listener, just wait for an implant to connect:
...
```
phantom (STUCK_ARTICLE) > wg-portfwd add --remote 10.10.10.10:3389
