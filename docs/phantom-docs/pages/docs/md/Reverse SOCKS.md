Phantom supports two types of SOCKS5 proxies, an "in-band" proxy that tunnels through any C2 protocol, and a WireGuard SOCKS proxy (only available when using WireGuard C2).
...
```
phantom (UGLY_SCARIFICATION) > socks5 add
...
Next, using Phantom you can create WireGuard client configuration using the `wg-config` command (you can use `--save` to write the configuration directly to a file):
