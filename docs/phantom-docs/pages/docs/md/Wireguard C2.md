...
Implant-side transport is implemented in `implant/phantom/transports/wireguard/wireguard.go`:
...
- Implant signs marshaled envelope payloads in `implant/phantom/transports/wireguard/wireguard.go` (`WriteEnvelope`).
...
- Implant verifies in `implant/phantom/transports/wireguard/wireguard.go` (`ReadEnvelope`) using embedded server minisign public key (`MinisignVerifyRaw`).
