This script installs the latest version of Phantom as a systemd service, installs Windows cross-compiler dependencies (mingw), and sets up multiplayer for all local users. After running the script, connect locally by running `phantom`.

https://phantom.sh/install
...
```
curl https://phantom.sh/install|sudo bash
```

- Installs server binary to `/root/phantom-server`
- Installs mingw
- Runs the server in daemon mode using systemd
- Installs client to `/usr/local/bin/phantom`
