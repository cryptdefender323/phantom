...
Server related logs are saved to: `~/.phantom/logs/`

- `phantom.log` is the main log file for server related events
- `console.log` is the log of the server's built-in "client" console
- `phantom.json` JSON formatted log (includes timestamps)
...
`~/.phantom-client/phantom-client.log`
