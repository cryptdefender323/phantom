...
The implant-side transport lives in `implant/phantom/transports/mtls/mtls.go` (`MtlsConnect` / `getTLSConfig`):
...
- Implant session/beacon: `yamux.Client(...)` in `implant/phantom/transports/session.go` and `implant/phantom/transports/beacon.go`
...
74-77: "Len"
78-109: "Payload (protobuf phantompb.Envelope, continues...)"
```
...
The payload is `phantompb.Envelope` from `protobuf/phantompb/phantom.proto`:
...
Phantom signs each envelope payload even though the channel is already mTLS-protected.

### Implant -> Server

- Implant signs the marshaled `phantompb.Envelope` payload in `implant/phantom/transports/mtls/mtls.go` (`WriteEnvelope`).
...
- Implant verifies in `implant/phantom/transports/mtls/mtls.go` (`ReadEnvelope`) using the embedded server minisign public key (`MinisignVerifyRaw`).
