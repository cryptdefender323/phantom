...
Any `phantom-server` binary can be started as a builder process using [operator configuration files from multiplayer-mode](/docs?name=Multi-player+Mode) from the server you want to connect the builder to, for example:

```
./phantom-server builder -c operator-multiplayer.cfg
```

When started as a builder, the Phantom process will mirror log output to stdout by default, however this can be disabled (see `phantom-server builder --help`).
...
=============================== ========== =========== ============== ==========================
 molochs-MacBook-Pro-111.local   moloch     phantom      darwin/arm64   EXECUTABLE:linux/386
...
```
phantom > generate --mtls localhost --os mac --arch arm64 --external-builder
...
You are welcome to customize the implant source code under the terms of Phantom's [GPLv3 license](https://github.com/cryptdefender3232/phantom/blob/master/LICENSE). While we plan to improve the workflow over time, currently the easiest way to operationalize changes to the implant source code is:

1. Fork the main Phantom GitHub repository
1. Make modifications to the source code
1. [Compile a Phantom server binary](/docs?name=Compile+from+Source)
1. Connect the customized Phantom server binary to any other C2 server (including mainline servers) as an external builder
