...
Phantom’s interactive consoles (both `phantom-client` and the server-only console) use a theme file to drive all lipgloss styling.

- Theme path: `~/.phantom-client/theme.yaml`
- Settings path: `~/.phantom-client/tui-settings.yaml`

If `theme.yaml` is missing, Phantom will create it with the default palette.
...
prompt_template: |
  {{- if .IsServer -}}
  {{ .Styles.Bold.Render "[server]" }} {{ .Styles.Underline.Render "phantom" }}{{ .Target.Suffix }} >
  {{- else -}}
  {{- if .Host -}}{{ .Styles.BoldPrimary.Render (printf "[%s]" .Host) }} {{- end -}}
  {{ .Styles.Underline.Render "phantom" }}{{ .Target.Suffix }} >
  {{- end -}}
```
...
- `operator-host`
  - `phantom-client`: `[operator@host] phantom >`
  - server console: `[server] phantom >`
- `host`
  - `phantom-client`: `[host] phantom >`
  - server console: `[server] phantom >`
- `basic`
  - both: `phantom >`
- `custom`
  - the entire prompt text is rendered from `prompt_template` (Go `text/template`)
  - if rendering fails, Phantom falls back to a minimal prompt: ` > `

### Custom Prompt Templates

When `prompt: custom`, Phantom evaluates `prompt_template` as a Go `text/template` every time the prompt is displayed.
