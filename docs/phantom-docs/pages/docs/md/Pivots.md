⚠️ **IMPORTANT:** Pivots in Phantom are used for specifically pivoting C2 traffic, not to be confused with port forwarding `portfwd`, which is used for tunneling generic TCP connections into a target environment.
...
In Phantom you use an existing session to create a "pivot listener" and then generate new pivots that can connect back to that listener, just as you would with other C2 protocols/endpoints.
...
```
[server] phantom (PRIOR_MANTEL) > pivots tcp

[*] Started tcp pivot listener :9898 with id 1

[server] phantom (PRIOR_MANTEL) > pivots
...
[server] phantom > use a2615359-b0a4-4463-820b-f4dbe6fb2c30
...
[server] phantom (WARM_DRIVEWAY) > sessions
