...
Phantom can send server event notifications via the `github.com/nikoksr/notify` services. Notifications are configured in `server.yaml` under the `notifications` block (located at `~/.phantom/configs/server.yaml` by default).
...
notifications:
  ...
  services:
    slack:
      enabled: true
      api_token: "$PHANTOM_SLACK_TOKEN"
      channels:
        - "C0123456789"
    telegram:
      enabled: true
      api_token: "$PHANTOM_TELEGRAM_TOKEN"
      chat_ids:
        - "123456789"
      events:
        - session-connected
...
notifications:
  ...
  services:
    http:
      enabled: true
      urls:
        - "https://hooks.example.com/phantom"
      webhooks:
        - url: "https://hooks.example.com/phantom-auth"
          method: "POST"
          content_type: "application/json"
          headers:
            Authorization: "$PHANTOM_WEBHOOK_TOKEN"
    syslog:
      enabled: true
      priority: "info"
      network: "udp"
...
By default this is `~/.phantom/notifications/templates/`. Only a filename is allowed (no path separators) and `..` is rejected to prevent path traversal.
