...
func TestAllMatchesEmbeddedMarkdownSources(t *testing.T) {
	dirEntries, err := os.ReadDir("phantom-docs/pages/docs/md")
	if err != nil {
	...
	for _, entry := range dirEntries {
		...
		content, err := os.ReadFile(filepath.Join("phantom-docs/pages/docs/md", entry.Name()))
		if err != nil {
...
func TestReadAcceptsDocNameWithoutSuffix(t *testing.T) {
	want, err := os.ReadFile(filepath.Join("phantom-docs/pages/docs/md", "Getting Started.md"))
	if err != nil {
...
func TestAsciinemaFSMatchesEmbeddedSources(t *testing.T) {
	dirEntries, err := os.ReadDir("phantom-docs/public/asciinema")
	if err != nil {
	...
	for _, entry := range dirEntries {
		...
		content, err := os.ReadFile(filepath.Join("phantom-docs/public/asciinema", entry.Name()))
		if err != nil {
...
func TestReadAsciinemaAcceptsNameWithoutSuffix(t *testing.T) {
	want, err := os.ReadFile(filepath.Join("phantom-docs/public/asciinema", "intro.cast"))
	if err != nil {
