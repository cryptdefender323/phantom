# Phantom AI Agentic Loop

## Goal

Port the example agentic loop from `examples/codex/codex-rs` into Phantom's AI chat so the server can:
...
Phantom ports the same shape, adapted to its existing RPC and database model.
...
Phantom now publishes `AIConversationEvent` payloads instead of raw `AIConversation` snapshots.
...
4. If the runtime supports the Responses API loop, Phantom runs the agentic loop.
...
- another Phantom client can attach to the same conversation and see the full turn history
...
- Phantom currently uses non-streaming Responses API requests, so reasoning items arrive as completed items rather than token deltas.
- Tool execution is currently a bounded read-only server-side function set, not the full Codex local tool surface.
- Phantom persists intermediate items in its conversation DB rather than only in a transient thread runtime.
