...
export PHANTOM_ROOT_DIR="$TEST_TMP_ROOT/phantom"
export PHANTOM_CLIENT_DIR="$TEST_TMP_ROOT/phantom-client"
export PHANTOM_CLIENT_ROOT_DIR="$PHANTOM_CLIENT_DIR" # legacy compatibility
...
mkdir -p \
	...
	"$GOCACHE" \
	"$GOTMPDIR"
export PATH="$PHANTOM_ROOT_DIR/go/bin:$PATH"

echo "Using isolated temp directories:"
echo "  PHANTOM_ROOT_DIR=$PHANTOM_ROOT_DIR"
echo "  PHANTOM_CLIENT_DIR=$PHANTOM_CLIENT_DIR"

print_failure_logs() {
	cat "$PHANTOM_ROOT_DIR/logs/phantom.log" 2>/dev/null || true
}
...
unpack_server_assets() {
	if [ -x "./phantom-server" ]; then
		if ./phantom-server unpack --force; then
			return 0
		fi
		echo "phantom-server unpack failed, falling back to go run ./server unpack --force"
	fi
...
if [ "$UNIT_ONLY" -eq 1 ]; then
	...
	run_test_cmd "./server/c2 (e2e yamux)" go test -tags="server,$TAGS,phantom_e2e" ./server/c2 -run 'Test(MTLS|WG)Yamux_' -count=1 || exit 1
	run_test_cmd "./server/c2 (e2e dns)" go test -tags="server,$TAGS,phantom_e2e" ./server/c2 -run 'TestDNS_' -count=1 || exit 1
fi
...
if [ "$SKIP_GENERATE" -eq 0 ]; then
	...
	GENERATE_GOMAXPROCS="${PHANTOM_GENERATE_GOMAXPROCS:-2}"
	GENERATE_GO_P="${PHANTOM_GENERATE_GO_P:-1}"
	GENERATE_TEST_PARALLEL="${PHANTOM_GENERATE_TEST_PARALLEL:-1}"
